...
final class EffectParameterCharacterizationTests: XCTestCase {
    ...
    private static let everyMenuEntry: [Effect] = [
        ...
        // TODO (88). One entry for three modes — `Glare`'s convention: the in-bar Mode picker is a
        // settings-bar concept and every mode shares this one table.
        .guide(Effect.Guide()),
    ]
    ...
    func testTheSlidersMatchTheSettingsBarCallSitesTheyReplaced() {
        ...
        // TODO (63). Every row exists whatever `type` is — the bar hides what a type does not use,
        // the table does not. `type` and `rotate45` are `.option`/`.boolean` and carry no `uiRange`,
        // so neither is here, the same reason `blur.directional` and `bloom.input` are not.
        XCTAssertEqual(sliderRows(.glare(Effect.Glare())), [
            "threshold|Threshold|0.0...1.0|%.2f",
            "intensity|Intensity|0.0...2.0|%.2f",
            "streaks|Streaks|2.0...16.0|%.0f",
            "angleOffset|Angle|0.0...180.0|%.0f°",
            "fade|Fade|0.0...1.0|%.2f",
    ...
    func testThereAreSeventySixRangedParametersInTheWholeCatalogue() {
        ...
        XCTAssertEqual(cases.count, 20, "Twenty cases behind twenty-three menu entries")
        // TODO (74)'s Lens Blur: radius, blades, threshold and boost are ranged; `input` is not.
        // TODO (88)'s Guide: eleven of its fourteen — `mode`, `color` and `twoPoint` are not.
        XCTAssertEqual(cases.flatMap { sliderRows($0) }.count, 76)
    }
    ...
    func testEveryAnimatableParameterCarriesAFormatForTheGraphEditorToRead() {
        ...
        // TODO (88): the Guide's nine doubles (spacing, angle, width, opacity, horizon, four
        ...
        XCTAssertFalse(animatable.contains("posterize.levels"), """
            ...
            """)
        XCTAssertFalse(animatable.contains("glare.streaks"), "PREMISE: the same shape, for the same reason")
    }
    ...
    /// **97 stored fields over 20 payload structs, and every one of them addressable.** 33 the day
    ...
    func testEveryStoredFieldOfEveryPayloadHasAnAddress() {
        let expected: [(Effect, Int)] = [
            ...
            // TODO (88): mode, spacing, subdivisions, angle, line width, colour, opacity, density,
            // horizon, two vanishing points of two coordinates each, and the two-point switch.
            (.guide(Effect.Guide()), 14),
        ]
        ...
        XCTAssertEqual(expected.map(\.1).reduce(0, +), 97)
    }
    ...
    private static func storedFieldCount(_ effect: Effect) -> Int {
        ...
        case .guide(let p):               return Mirror(reflecting: p).children.count
        }
    }
    ...
    func testTheIdsAreTheirWholeStableList() {
        ...
        XCTAssertEqual(Set(Self.everyMenuEntry.flatMap { ids($0) }).sorted(), [
            ...
            "glare.angleOffset", "glare.fade", "glare.intensity", "glare.length",
            "glare.rotate45", "glare.size", "glare.streaks", "glare.threshold", "glare.type",
            "gradientMap.mix", "gradientMap.stops",
            "guide.angle", "guide.color", "guide.density", "guide.horizon", "guide.lineWidth",
            "guide.mode", "guide.opacity", "guide.spacing", "guide.subdivisions", "guide.twoPoint",
            "guide.vanishingPoint1X", "guide.vanishingPoint1Y",
            "guide.vanishingPoint2X", "guide.vanishingPoint2Y",
    ...
    /// **Thirteen structural fields hold, and these are they.** TODO (63)'s Glare added three:
    /// `glare.type` is a formula like `duplicateOffset.region` — Streaks, Simple Star and Fog Glow
    /// are three different pass lists, nothing between them; `glare.streaks` is an `Int` like
    /// `posterize.levels`; `glare.rotate45` is a `Bool` like `noise.monochrome`. TODO (61)'s
    /// Duplicate Offset added two before it: its region and its blend mode are each a formula, not a
    /// quantity — half a rim is not a region and there is nothing between Multiply and Screen. Three
    /// of the original eight change the render *shape* rather than a number — `blur.directional`
    /// rewrites the pass list from two passes to one, `bloom.input` decides whether the compositor
    /// performs an entire sub-walk into two borrowed textures, and TODO (60)'s `hsvShift.colorize`
    /// swaps what `hueDegrees`/`saturation` mean and the effect's own `displayName` (its own doc: the
    ...
    /// `noise.monochrome`.
    func testTheNineteenSteppedParametersAreTheStructuralOnes() {
        ...
        XCTAssertEqual(Set(stepped), [
            ...
            // TODO (88): an enum, two `Int`s and a `Bool` — a grid with two and a half subdivisions
            // is not a grid, and half a second vanishing point is not one.
            "guide.mode", "guide.subdivisions", "guide.density", "guide.twoPoint",
        ])
    }
    ...
    /// contribute.
    func testSeventyFiveParametersAreContinuous() {
        ...
        // TODO (88)'s Guide: nine doubles and its line colour, the fourth.
        XCTAssertEqual(continuous.count, 75)
        // Bloom precedes Outline precedes Duplicate Offset precedes the Guide in `everyMenuEntry`.
        XCTAssertEqual(continuous.filter { $0.value == .colour }.map(\.id),
                       ["bloom.color", "outline.color", "duplicateOffset.color", "guide.color"])
        XCTAssertEqual(continuous.filter { $0.value == .double }.count, 71)
    }
    ...
    /// keyframed ramp comes out in integer steps. It is not only the blur's — bloom's, sharpen's and
    /// TODO (63)'s Glare's `length` all reach the same function (Fog Glow's `size` does not: it never
    /// reaches `tapCount` at all, since it delegates straight to `Bloom.radius`, itself already here).
    func testFourRadiiAreRoundedByTheRenderPath() {
        let rounded = Self.everyMenuEntry
            .flatMap { $0.parameters }
            .filter { $0.quantisation == .roundedByTheRenderPath }
            .map(\.id)
        XCTAssertEqual(Set(rounded), ["blur.radius", "bloom.radius", "sharpen.radius", "glare.length"])
    ...
    func testTheIntegralParametersAreTheOnesWhoseFieldIsNotADouble() {
        ...
        XCTAssertEqual(Set(integral), [
            ...
            "lensBlur.blades", "lensBlur.input",
            // TODO (88): an enum, two `Int`s and a `Bool`.
            "guide.mode", "guide.subdivisions", "guide.density", "guide.twoPoint",
        ])
    }
