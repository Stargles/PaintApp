...
//
// This file previously held a single VectorShapeAndRecoveryUITests class (12 tests, 389s
// measured). It is now three classes so xcodebuild's per-class parallel scheduling can put
// them on separate simulator clones instead of one clone carrying the whole 389s alone. See
// GalleryRecoveryUITests below for why the two gallery/corruption tests must stay together.
...
final class GalleryRecoveryUITests: PaintUITestCase {
...
/// Vector-layer content tests: geometry recording, rasterize, palette persistence, and the
/// layer-transform-applied-twice regression. None of these mutate the gallery or palette store in
/// a way that needs to be serialized against another class — `testPaletteBuilderAddAndSelectSwatch`
/// passes `-resetPalettes`, a distinct persistent store from the `-resetGallery` project gallery
/// that GalleryRecoveryUITests above is isolated for, and each simulator clone has its own
/// container, so this class running on a separate clone from GalleryRecoveryUITests cannot race it.
final class VectorLayerContentUITests: PaintUITestCase {
...
/// Raster-layer shape lifecycle tests: baking, repaint-on-undo/redo, drawing over a pending shape,
/// and dragging a shape's handles. Independent of the gallery/palette persistent stores above, so
/// this class is free to run on any simulator clone.
final class ShapeRecoveryUITests: PaintUITestCase {
