...
final class ToolLogicTests: XCTestCase {
    ...
    // MARK: - The fourth exclusion list, and the one this file was extended for
    //
    // `CanvasManager.selectedTool`'s `didSet` closes an engaged whole-layer vector Move on a tool
    // switch (BUGS.md "Move with no selection blocks the brush button", 2026-08-27). Written as a
    // bare "did the tool change" check, it would also fire on `selectEyedropper`/`leaveEyedropper` —
    // each one write to `selectedTool` — closing the Move on arming the pick *and* again on
    // returning from it. `Tool.isMomentary` is the exhaustive answer, same arrangement as the other
    // three tables in this file.

    /// Every case's answer, stated once, here — same arrangement and same reasoning as
    /// `expectedPaintsOnCanvas` and `expectedFollowsBrushPreset` above.
    private let expectedIsMomentary: [Tool: Bool] = [
        // The one case that matters: arming and leaving the eyedropper are both a `selectedTool`
        // write, but the artist experiences the pair as a single tap, not two tool changes.
        .eyedropper: true,
        // Every other tool is the artist's ongoing choice until they make a different one.
        .pen: false,
        .pencil: false,
        .eraser: false,
        .fill: false,
        // Unlike the eyedropper, text opens a session that outlives its placement tap, so it is
        // treated as a real switch even though the same arbitration protects that one tap —
        // `Tool.isMomentary`'s own doc comment has the reasoning.
        .text: false,
    ]

    func testEveryToolStatesWhetherItIsAMomentaryDetour() {
        XCTAssertEqual(Tool.allCases.count, expectedIsMomentary.count, """
            A case has been added to `Tool` without an entry in `expectedIsMomentary`. Decide whether \
            selecting it is a momentary detour the artist does not experience as a tool switch \
            (`true`) or an ongoing choice (`false`), then say so in `Tool.isMomentary` and in the \
            table above. Defaulting to `false` reproduces the eyedropper's own bug: a colour pick \
            would end an engaged whole-layer Move on the way in and again on the way back.
            """)

        for tool in Tool.allCases {
            guard let expected = expectedIsMomentary[tool] else {
                XCTFail("\(tool) has no stated answer — see the message on the count assertion")
                continue
            }
            XCTAssertEqual(tool.isMomentary, expected, "\(tool).isMomentary must be \(expected)")
        }
    }

    /// The owner's report, named directly, the same way `testTheEyedropperDoesNotPaintOnTheCanvas`
    /// pins `paintsOnCanvas`'s table against a rewrite.
    func testOnlyTheEyedropperIsMomentary() {
        XCTAssertTrue(Tool.eyedropper.isMomentary)
        for tool in Tool.allCases where tool != .eyedropper {
            XCTAssertFalse(tool.isMomentary, "\(tool) is an ongoing choice, not a detour")
        }
    }
}
