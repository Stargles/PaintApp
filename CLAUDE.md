...
[RENDER.md](RENDER.md) is the rendering specification — TODO item (29), the background baker that replaces live main-thread compositing, the on-disk frame store, and export. **Its §2 is fourteen owner rulings; read them rather than re-deriving them.** The knob is the truth (§2.12), so `CompositorBudget.affordableSize`'s silent shrink is a defect under it.
[KEYFRAMES.md](KEYFRAMES.md) is the keyframe-animation specification — TODO item (21), designed 2026-08-28. **Its §2 is twenty-eight owner rulings; read them rather than re-deriving them, and §8 is the build order.** Three of them (§2.1, §2.23, §2.24) are **superseded and kept**: the owner reversed their own Animate-mode ruling on 2026-08-29 and §2.26/§2.27 replaced it with the keyframe-mark workflow — a keyframe is a bare mark in time, an edit between two marks holds its previous value, and the *next* mark commits it. §2.28 then settles what "a keyframe" *is* against the model: the **union** of the explicit marks and every frame a channel keys on, computed by one accessor and never stored twice — two device bug reports were the same divergence between those two lists. The three that shape everything else: a transform key stores a **quad** from day one so Distort lands later with no migration; posed ink is drawn by **baking the dab walk in rest space** and mapping dab centres per frame, which removes the shimmer *and* the per-sample-width problem together; and **bake is an authoring feature, never a performance instruction** — smooth playback comes from a **span-scoped, eager, disk-backed** cache (§4.6), which is new machinery rather than a budget change, because the composite cache that exists today holds exactly one generation. Two traps it carries that reach past this feature: `PixelOps.rasterize`'s memo has no pose field while `SandwichKey` compares the whole node tree, so a posed frame composites happily from a stale un-posed image — green tests, wrong pixels, no key that looks wrong; and brush grain is a canvas-space noise field, which is invisible under a one-off Move and **boils at 24 fps**.
