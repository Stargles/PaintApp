...
Six things are worth knowing that the sketch above does not say:
...
- **Distort is a mode, not a modifier**, and `TextFrame.mode` is derived rather than declared. The four corner grips move independently only while the panel says Distort; the four edge grips and the rotation knob are unchanged in both modes, so all 27 of stage 4's identities stand. After each delta the mode is recomputed from the quad — a distort that lands back on a parallelogram goes back to `.affine`, or the box would lose the native unresampled drawing path for a warp it no longer has. `TextCornerMode` is tool state on `CanvasManager`, never on the document.
- **`w > 0` overlaps convexity, by theorem, and the test says so.** A projective map carries the unit square to a convex quad exactly when the square stays on one side of the vanishing line, so every quad `w > 0` rejects is also non-convex. Both are checked anyway: they are different questions (one about the polygon, one about the map), and a later Move Distort composing this solver with another transform can reach a matrix no quad was ever examined against.
...
**Explicitly not yet:** mesh/envelope warps, outline conversion, WYSIWYG convergence between the CA preview and the kernel bake. `TextMeasure.inkBounds` still reports the bounding box for a warped frame, on purpose — larger is the safe direction for the eraser's residue collector.
**Tests:** 28 new — 13 in `HomographyLogicTests`, 4 in `WarpAgreementCharacterizationTests`, 11 added to `TextTransformLogicTests`. Fast tier 1655 → 1683 (1680 passed, 0 failed, 3 skipped). Every one was watched failing on a named assertion before it was believed.
