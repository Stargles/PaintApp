...
Stage 4 left it a clean seam: `TextFrame.affineTransform` returns nil for any quad that is not a parallelogram and every drawing path already branches on that, so `affine()` on the new solver is expected to return the same matrix for every quad stage 4 can make, and `Homography` only has to cover the ones it cannot.
