...
**Not headless, and it needs the owner's iPad.** Whether a handle actually looks 14 pt at 0.3× zoom, and whether nine targets on a small box are pickable with a fingertip rather than merely with the arithmetic. Also unexercised off-device: a rotated `UITextView`'s caret, selection handles and Scribble, which is the concession §1 predicts and stage 1 already owed a device check for.
