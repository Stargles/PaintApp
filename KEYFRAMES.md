...
| **5b** | **Real Distort** — the *animated* one, a projective quad keyed across frames | §2.13. **Not to be confused with the Move-box Distort, which now ships on both tiers** (LASSO_MOVE §0): a raster floating piece since 2026-09-02 and a lassoed drawing since 2026-09-06, each reshaped by a live gesture. Nothing keys a projective quad over time — `TransformKeyframes` only ever writes a `PoseQuad` built from a `CGAffineTransform`, and `PoseQuad.affineOrLinearised` treats a projective pose as something only a document *this* stage wrote could contain. **The engine half is built and tested**: `VectorCanvas.posing(_:through: Homography)` is §4.2's point map with a projective pose, `CanvasManager.restDelta` has a projective overload, `composedWalk` composes one pose onto another, and `VectorStroke.distort` persists one. What is left is the *quad*: store a genuinely projective `PoseQuad`, route `TransformTrack`'s two render reads through `DabPose(Homography)` rather than `affineOrLinearised`, blend two projective quads in `PoseInterpolation`, and lift `distortUnavailableReason`'s container-pose arm. Placed images stay behind Move stage 3c and are refused out loud until they have a stored quad of their own. |
