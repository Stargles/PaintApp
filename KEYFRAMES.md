...
one line of what consumes it. **Confirmed 2026-09-06: stage 5b touched neither `DabPose` nor
`BrushStamper`.**
...
~~**Distort is refused on a container float and says so.**~~ **Superseded 2026-09-06 by stage 5b, and
the refusal was worse than it read.** The reason it gave was real — `affineOrLinearised` would have
linearised the pose at the box centre — but the mode picker stayed live and nothing stopped the gesture:
the corner drag wrote `distortQuad`, the outline foreshortened under the finger, `containerPose(_:movedBy:)`
took the *affine* delta, and the canvas did not follow. A caption is not a refusal. Stage 5b deleted the
linearisation, so `containerPose` carries the residue and `distortUnavailableReason` says nothing about a
container any more. **What is still refused is a kind** — a placed image or a video — and that is per
float, out loud, exactly as it was.
...
- **There is no multi-frame composite cache to extend — this is new machinery.** The sandwich holds
  **exactly one generation** (three canvas-sized images in `sandwichFull`/`sandwichHalves`, renamed
  since; `CanvasView.swift:1779`),
...
the frame-walker once. **Refuted by the build (2026-09-12)**: the exporter walks *pixels* and a bake to
cels walks *poses* — see §8's row.

**BUILT 2026-09-12** — `CanvasManager.bakePoseToCels` (`Models/CanvasManager+PoseBake.swift`), the
**Bake Animation** row on the cel menu beside Add Keyframe, shown only on a block that carries a pose
channel; `PoseBakeLogicTests` and `PoseBakeUITests`. What the build settled, beyond the rules above:

- **Where it cuts is where the picture changes, resolved on the uncut track.** A new cel starts at
  every frame whose resolved maps differ from the previous frame's — the step grid when the pose is
  moving (twelve frames on twos are six cels), and one cel for a hold past the last key, so a bake
  never mints two cels of one picture. Resolved *before* the first `splitCel`, because §3.1's split
  re-parameterises a bezier segment on both sides (MEASURED by mutation: on a bezier pair every middle
  frame changed when the halves were re-read; on a linear pair none did).
- **The pin — a baked frame is byte-identical to the animated frame on both backends, before a save
  and after a reload — needed a persisted form of §4.2's rest walk, and it is `StrokeDistort.rest`.**
  `posing`'s `restWalk` is transient; the affine Move commit re-walks in posed space (§4.2's finding
  4); the projective commit's pull-back through the map's inverse is a floating-point round trip
  that dropped the last dab on three of twelve frames of an integer fixture (MEASURED, 4e-15 pt of
  error on a knife edge) and would be an eighth of a pixel off after a save, since `samples` are
  written quarter-pixel quantised. So `VectorCanvas.baking` takes the projective commit for an affine
  map on purpose and keeps the artist's own spine beside the map, packed on disk exactly as the
...
- **"On a selection of the timeline" is not built**, because there is no cel selection — TODO (22).
- **What driving it found**: undoing the bake showed the drawing at its *stored* position on a frame
  whose diamonds said it was posed, until a scrub. `StrokeCanvasView.finishVectorRender` installed a
  landed render of the cel's own ink without reading `VectorPreviewPlan`, and the bake's undo is the
  first thing that bumps the version of the canvas the host is about to show in the same pass that
  installs the posed preview. `DeferredVectorRender.landing` refuses a landing under a derived base
  now. The fast tier, both backends, byte for byte, could not see it — CLAUDE.md's rule again.
...
| **6** ✅ | **Bake to cels** — merged 2026-09-12. | §6. **It does not share (29)'s frame-walker after all**: `FrameExportSession` walks *pixels* out of the bake store, and a bake to cels needs the *pose* the cel resolves at each of its own frames (`poseMappings`) to write posed geometry — the walker is `CanvasManager.poseBakeSegments`, over the cel's own tracks, and the exporter has no use for it. `Models/CanvasManager+PoseBake.swift`. |
...
| **7** ✅ | **Live recording + editable fps** | §5. **This row said *"nothing else of it exists — `fps` is fixed at 24"* and had been stale since 2026-09-07.** Built: the editable rate (clamped 1-60, live during playback, no undo step), §5.1's two-act arming, and all three surfaces — the effect and opacity sliders, the canvas (§7, stage 10), and **the Move box, 2026-09-10, §5.2**. **§5 is complete**: its last sentence, *"slow motion is a capture-speed multiplier on the record control"*, was **declined by the owner on 2026-09-10** because the editable rate already is one — the playback tick is `1.0 / fps` and a take resamples one stop per document frame, so recording a scene at 12 fps gives exactly twice the wall clock to perform in, with no control to build. See §5.3. |
...
**That prerequisite is discharged, and the *unkeyed* projective case shipped on it 2026-09-06**
(LASSO_MOVE §0, TODO item (12)). `VectorCanvas.mapping(_:through: Homography)` maps a stroke's spine and
lattice point for point and leaves the dab walk in rest space, so `PosedDabTarget` asks `localScale` and
`rotation` per dab; `InkDistortLogicTests.testEveryDabOfAKeystonedStrokeTakesTheLocalScaleAtItsOwnRestCentre`
pins each dab against the map at its own rest centre, which no single-scalar implementation can satisfy at
both ends. **This paragraph's own warning is worth keeping in front of stage 5b**: the refusal it justified
outlived it by four days, because nothing re-read it on the day the prerequisite merged.
...
Undo is `testATapAddsAKeyToTheCurveItLandsOnAndTheNodeMenuRemovesOne` (renamed since; TODO(45)), end
to end through a finger.
...
### 5.3 Slow motion is the rate, not a multiplier — declined 2026-09-10
