...
1. **Mask threshold and antialias constants** (§6.3) — `AlphaMask.threshold` (0.5) and
   ...
   brush on the iPad, where §6.4's `-Onone` cost is also felt — the two sliders are in the layer
   options menu (`MaskTuningSection`, temporary, deleted whole once the constants are chosen). They
   were a floating panel in the canvas's top-trailing corner until it turned out that corner belongs
   to the layer rail and every trailing dropdown, and a view declared last in that `ZStack` hit-tests
   above them. "Clip to below" itself shipped in 6a, as a mask whose source is implied.
