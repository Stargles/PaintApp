...
/// event.** `allowableMovement` is `UILongPressGestureRecognizer` API and has no counterpart here;
/// a tap's internal slop is undocumented and generous, and the owner's recordings have one
/// recognizing after 25 and 27 pt of travel. So a handler that asks *"was this touch on X?"* cannot
/// ask it at `.ended` of any point at all — not the release point, and not the touch-down point
/// either, because by then the drag has moved X. Both Move boxes therefore answer that question at
/// touch-down, in their recognizer's `gestureRecognizer(_:shouldReceive:)`, and refuse the touch;
/// what this subclass carries is only the touch type. A handler asking *"where did the artist
/// point?"* — `SelectionOverlayView.handleTap`'s automatic-selection tap — may read either end and
/// is deliberately left reading `location(in:)`.
