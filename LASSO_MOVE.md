...
| which rule a **recolour** — and since TODO (42) a size or an opacity edit — follows | `CanvasManager.beginSelectionEdit` (`CanvasManager+SelectionEdit.swift`) — the one branch the three rules cost. `.cutting` takes `splitForLassoMove` and rewrites `insideIDs`; the other two take `elementIDs(insideLoops:membership:)` and rewrite whole. §5.26 |
