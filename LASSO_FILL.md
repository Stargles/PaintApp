...
| 2 | **Loop drawn entirely inside a solid/dark area** | **Fills nothing** + the signal from §7. | The ring's reference colour becomes that solid colour, the flood consumes the whole interior, nothing is held out. Same code path as case 6. |
...
- **A gap wider than `g` silently produces nothing.** This is unsolved industry-wide and we are not going to solve it; the mitigation is entirely the §7 signal. Expect it to be the top support question.
