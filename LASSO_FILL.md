...
**Two earlier specifications of this step were wrong, and the measurement that killed the first is what fixes the third.** `G = 0` was the spec until 2026-08-21, on the claim that a fill covering the line has no seam to hide; an ordinary dilate by `G` replaced it the same morning. The table in `testTheFillsSoftEdgeComesFromTheArtworksOwnAntialiasing` is the evidence in both directions: step 6 gives the artwork's outer fringe a coverage of 0/213/255 against an artwork alpha of 0/64/160, so at `R = 0` the fill reaches exactly as far as the ink and **not one pixel further** — which is precisely *"on the high setting it is on the outer edge"*, and is why the top of the range is where it is rather than anywhere else.

The same table states the price, and it is worth stating because the next reader will otherwise re-file it as a bug: at that fringe pixel the composite is 64 over 213, i.e. alpha 223, so ~12% of the background shows through one pixel of the outline. Closing *that* requires the fill's own fade to land on paper the ink does not occupy, and the fade has only three places it can be — on the paper (rejected here), under the ink (what ships), or nowhere, by cutting hard at the ink's outer extent, which trades the halo for jaggies and throws away step 6. Measured filled fraction of the canvas at slider 0…6 on the closed-box fixture, 2026-08-22: 0.2906, 0.3077, 0.3253, 0.3433, 0.3619, 0.3809, **0.4005** — the last being `boxFootprint` to four places, i.e. exactly the shape the artist drew.

Two consequences worth naming. The erosion also pulls the fill back from the **fence**, because out-of-loop pixels read 0; that cannot violate §3's *"the fill shouldn't even touch the loop"* — shrinking never crosses a boundary — and a loop drawn through ink gets the same retreat there it gets at the ink. And the §6 step 5 pixel count **is** retaken after this step, which the dilate did not have to do: growth could never empty a non-empty result, but an erosion can (lasso a 5 px blob at the bottom of the slider), and leaving the count at step 5's answer would bake an invisible fill and book an undo entry for it — exactly what §7.1 forbids.
