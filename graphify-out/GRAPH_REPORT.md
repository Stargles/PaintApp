# Graph Report - PaintSoftware  (2026-09-02)

## Corpus Check
- 334 files · ~1,586,504 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 10939 nodes · 34409 edges · 319 communities (297 shown, 22 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 3440 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5802ef19`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- PaintUITestCase
- ShapeGeometry
- ProjectBackupManager
- .manager
- Coordinator
- PointCloudIndex
- bash
- CanvasManager
- Homography
- ShapeOverlayView
- ValueLayerLogicTests
- cels
- StrokeCanvasView
- ActionRecorder
- TimelineGraphBand
- CGPoint
- UIKit
- KeyframeTarget
- VectorEraserLogicTests
- LassoMoveLogicTests
- .rasterize
- .activeCelIndex
- CanvasManager
- CodingKeys
- Effect
- .drawLine
- TextHitTestLogicTests
- CanvasManager
- PerfBaselineTests
- Fill.metal
- Lattice
- StrokeScratch
- HistoryActionLabel
- ActivePanel
- Binding
- VectorFloat
- FloatingPieceOverlayView
- AnimationTimeline
- View
- ProjectSaveLogicTests
- LayerStackCell
- VectorCanvas
- ColorPickerPanel
- SelectionOverlayView
- VectorCanvasData
- FrameExportSession
- ContentView
- PerfMonitor
- StrokeGeometryLogicTests
- LayerManifest
- CanvasSizePickerView
- TextRecipe
- TimelineKeyMarkersLogicTests
- ObjectTransformFrame
- TextOverlayView
- FrameExportLogicTests
- Housekeeping pass (8 items from 2026-07-22 feature audit)
- XCUIApplication
- RenderRequest
- BrushEngineLogicTests
- AlphaMask
- ProjectStore
- FrameRecipe
- SideToolbar
- CompositorParityLogicTests
- CanvasManager
- EffectSettingsBar
- .assertChunkedMatchesWholeOnGPU
- GalleryOpenState
- SaveDamageGateLogicTests
- PaintSoftware iPad drawing/animation app
- BrushStamper.DabRNG (seeded splitmix64)
- PerfBaselineTests.swift
- LayerTreeCharacterizationTests
- ColorMathOklabLogicTests
- ObjectTransformOverlayView
- CompositorMetalEngine
- KeyframeControlLogicTests
- The layer transform — keep it, or bake it into the geometry?
- CodingKeys
- .makeUIView
- Equatable
- SandwichLogicTests
- CGImage.cropping(to:) retains parent pixel data
- Multi-Session Protocol
- graphify usage protocol
- VectorCanvasDataLogicTests
- TextBakeCharacterizationTests
- InterpolationMotionGroupLogicTests
- parallel_test.sh
- .solidImage
- DeformFactorization
- EffectMultiPassLogicTests
- .evaluate
- InterpolationRenderLogicTests
- Layer Compositing
- cleanup_session.sh
- screenshot.sh
- Switching brush presets resets live size/opacity
- Duplicated canvas-flip geometry (flippedImage/RasterLayerTexture.flipped)
- graphify-guard.sh
- fast_test.sh
- status.sh
- Raster-layer ghost-layer bug (Session 49)
- ShapeDetector unified-scoring rewrite (Session 51)
- ContentView.saveIfNeeded gap (direct project transition)
- Distort/Warp transform modes render identically to Uniform
- Adjust panel / ActionsMenu Cut-Copy-Paste stubs
- BlendMode
- ObjectTransformDrag
- GuideStroke
- float3
- PlaybackBoundsCharacterizationTests
- GuideOverlayView
- BrushBlendMode
- LassoFillLogicTests
- GuidePath
- EffectParityLogicTests
- AnimationCurve
- .group
- .apply
- VectorSample
- InterpolationGuideLogicTests
- TextTransformOverlayView
- command
- Compositor.swift
- FontDescriptor
- CanvasManager
- linear_light_q1q2.swift
- Typography
- Composite.metal
- EyedropperLogicTests
- Codable
- GuideRow
- InterpolateBar
- XCUIApplication
- EffectParams
- VectorEraserHybridLogicTests
- BakeKeyEncoder
- CanvasResizeLogicTests
- .launchIntoEditor
- CanvasNotice
- .assertLinear
- OnionSkinSettingsSource
- 4. Future upgrades — the deferred list
- CaseIterable
- Kind
- FillGestureRestartLogicTests
- LayerStackListView
- CanvasManager
- PinchMergeGateLogicTests
- ExportSheet
- .rasterize
- RasterLayerTexture
- Gesture
- CanvasTransformFreezeUITests
- ShapeHoldClock
- run.sh
- CanvasTouchInputs
- Keyframes
- CanvasManager
- .lassoFill
- CurveEditor
- OnionSkinPanel
- EffectParameter
- Recording
- StrokeSpatialIndex
- RenderNode
- BakeQueueLogicTests
- VectorPreviewPlanLogicTests
- 1. The decisions
- EffectParameterTrackLogicTests
- Int
- TextLayout
- CodingKeys
- .setUpGestures
- .compositeSize
- MetalFillSession
- OnionSkinSettings
- 0. What already exists
- TODO.md
- Is the brush engine ready for `.ABR` / Procreate brush import?
- TransformChannelID
- WarpAgreementCharacterizationTests
- AnimationCurveLogicTests
- simlock.sh
- CanvasTouchOwnerLogicTests
- .rgbaBytes
- EffectPipelines
- ObjectTransformLogicTests
- FillBoundaryLogicTests
- CanvasResizeSheet
- TextSettingsPanel
- String
- UndoHistory
- Performance
- PlacedImageShapeLogicTests
- UInt8
- TimelineLayoutKeyLogicTests
- CanvasHostView
- Lasso Fill — Specification
- InterpolationEngineDiagnosticsLogicTests
- VideoFrameWriter
- TimelineRowLayoutLogicTests
- CelContentProviderLogicTests
- XCTestCase
- Layer
- SizePreviewWindow
- linear_light_ab.swift
- CanvasTouchOwner
- CanvasPresentation
- MenuInterruptionUITests
- BlockDragCharacterizationTests
- Effect backdrop — what an adjustment layer grades
- LayerStackRow
- TimelineGraphChannelListLogicTests
- SpacingChart
- Every dismissible presentation, and whether a stroke under it breaks
- Group
- TransformTrack
- Brush
- .handleReorderDrag
- Open
- CGFloat
- TimelineLayoutKey
- DecodedFrameRing
- 1. The decisions
- presentation-census.sh
- Foundation
- FrameBaker
- FrameBakerLogicTests
- FrameBakeStoreLogicTests
- WarpParams
- .previewed
- OnionSkinLogicTests
- CodingKeys
- TimelineBakeBar
- 1. What will actually hurt, ranked
- SelectPanel
- ARAPLogicTests
- PoseQuad
- MemoryBudgetLogicTests
- 3. Design
- .assertChunkedMatchesWhole
- ProjectLoadDamage
- TransformChannelLogicTests
- .draw
- FrameBakeStore
- StructureSnapshot
- DerivedCelContent
- StrokeStabilizer
- String
- DabTarget
- ViewPreset
- CanvasLayerFilterLogicTests
- CanvasGeometryLogicTests
- StrokeGestureRecognizer
- .testTheBarIsBlankOnceTheDocumentHasBaked
- DrawingView
- CanvasActiveLayer
- Linear-light A/B — the picture for TODO item (10)
- Coordinator
- BakeWiringLogicTests
- Graph editor — screenshots
- WindowEventTap
- CanvasPresentationModifier
- PoseWidthAB
- LayerStackListView.Coordinator
- Failure
- BakeFixture
- Handoff
- DistortLogicTests
- FontResolveLogicTests
- Phase
- Coordinator
- JSONValue
- CodingKeys
- .sample
- Atomic
- CodingKeys
- TransformOverlaySupport.swift
- .canvas
- AnimationGroup
- Alignment
- Resolution
- CGContextDabTarget
- ManifestSkeleton
- Kind
- AppliedTool
- Neighbourhood
- Placement
- Zone
- .inkBounds
- DistortUITests
- CodingKey
- Admission
- Kind
- LiftKind
- 2. The decisions
- FillParams
- .safeStem
- .draw
- .sampledColor
- .row
- .init
- CodingKeys
- RecordingDabTarget
- Result
- RenderResolution
- LayerTransform
- .handleShouldReceive
- CanvasManager
- .testTheCanvasComesBackToTheBakedFrameAfterAStroke
- LayerRowModel
- PackedSampleRun
- .testGrainTravelsWithTheInkUnderAPose
- RecordingWriter
- RestSpaceDabBakeLogicTests

## God Nodes (most connected - your core abstractions)
1. `CGPoint` - 1185 edges
2. `CGFloat` - 881 edges
3. `Effect` - 283 edges
4. `VectorCanvas` - 277 edges
5. `CanvasManager` - 192 edges
6. `VectorSample` - 185 edges
7. `LassoMoveLogicTests` - 164 edges
8. `Coordinator` - 131 edges
9. `ShapeGeometry` - 121 edges
10. `XCTest` - 116 edges

## Surprising Connections (you probably didn't know these)
- `.fixedBrush` --calls--> `Brush`  [EXTRACTED]
  PaintSoftwareUITests/StrokeGeometryLogicTests.swift → PaintSoftware/Engine/Brush.swift
- `.sourceScale` --calls--> `CGFloat`  [EXTRACTED]
  PaintSoftwareUITests/WarpAgreementCharacterizationTests.swift → PaintSoftware/Engine/Deform/Lattice.swift
- `.centre` --calls--> `CGPoint`  [EXTRACTED]
  PaintSoftwareUITests/PoseInterpolationLogicTests.swift → PaintSoftware/Engine/Deform/MotionGrouping.swift
- `StrokeGeometryLogicTests` --calls--> `CGPoint`  [INFERRED]
  PaintSoftwareUITests/StrokeGeometryLogicTests.swift → PaintSoftware/Engine/Deform/MotionGrouping.swift
- `.quads` --calls--> `CGPoint`  [INFERRED]
  PaintSoftwareUITests/WarpAgreementCharacterizationTests.swift → PaintSoftware/Engine/Deform/MotionGrouping.swift

## Import Cycles
- None detected.

## Hyperedges (group relationships)
- **Stroke-delivery regression: default flip, gate, and recognizer state involved together** — bugs_stroke_delivery_regression, bugs_requirespencilonly, bugs_pencilonlydrawing, bugs_reconcilelayers, bugs_housekeeping_2026_07_26 [EXTRACTED 1.00]

## Communities (319 total, 22 thin omitted)

### Community 0 - "PaintUITestCase"
Cohesion: 0.07
Nodes (17): AuthoredCurve, GraphEditorUITests, PaintUITestCase, Double, Int, XCUIApplication, HistoryNoticeUITests, SelectionPencilOnlyUITests (+9 more)

### Community 1 - "ShapeGeometry"
Cohesion: 0.05
Nodes (27): Corner, bottomLeft, bottomRight, topLeft, topRight, Edge, bottom, left (+19 more)

### Community 2 - "ProjectBackupManager"
Cohesion: 0.07
Nodes (29): DateFormatter, Notification.Name, ProjectBackup, .id, ProjectBackupManager, .backupsRootDirectory, .currentAppSignature, .documentsDirectory (+21 more)

### Community 3 - ".manager"
Cohesion: 0.05
Nodes (12): CanvasFixture, CanvasManager, CGSize, Int, CelCRUDCharacterizationTests, CanvasManager, Int, SelectionPersistenceLogicTests (+4 more)

### Community 4 - "Coordinator"
Cohesion: 0.05
Nodes (47): BlockDrag, CelBlockView, Coordinator, .visibleBandX, GraphBandDrag, Kind, cel, gap (+39 more)

### Community 5 - "PointCloudIndex"
Cohesion: 0.11
Nodes (19): ARAPRegistration, Constraint, Matching, bidirectional, sourceToTarget, Options, PointCloudIndex, .isEmpty (+11 more)

### Community 6 - "bash"
Cohesion: 0.07
Nodes (48): agent, orchestrator, worker-bugfix, worker-feature, worker-integration, worker-research, worker-test, worker-ui (+40 more)

### Community 7 - "CanvasManager"
Cohesion: 0.03
Nodes (74): Void, CanvasManager, .activeContainerID, .activeLayerKind, .availableBrushes, .availableEraserBrushes, .canvasPaddingRange, .contentEndFrame (+66 more)

### Community 8 - "Homography"
Cohesion: 0.05
Nodes (38): Homography, .catransform3D, .determinant, .inverse, Bool, CATransform3D, CGRect, CGSize (+30 more)

### Community 9 - "ShapeOverlayView"
Cohesion: 0.06
Nodes (39): CornerHandle, EdgeHandle, EndpointHandle, end, start, HandleInfo, HandleKind, axisBottom (+31 more)

### Community 10 - "ValueLayerLogicTests"
Cohesion: 0.17
Nodes (7): CanvasManager, CGImage, Int, UIColor, UIImage, ValueLayerLogicTests, .side

### Community 11 - "cels"
Cohesion: 0.11
Nodes (11): baked, cels, InterpolationReferenceOnionSkinSource, InterpolationWorkflowLogicTests, Bool, CanvasManager, Cel, Int (+3 more)

### Community 12 - "StrokeCanvasView"
Cohesion: 0.07
Nodes (31): CAShapeLayer, StrokeInput, TimeInterval, StrokeSampleGate, Bool, NSCoder, StrokeCanvasView, .brush (+23 more)

### Community 13 - "ActionRecorder"
Cohesion: 0.16
Nodes (12): ActionRecorder, .directory, .now, CFTimeInterval, CGSize, Double, ObjectIdentifier, String (+4 more)

### Community 14 - "TimelineGraphBand"
Cohesion: 0.07
Nodes (26): Channel, .axis, Colour, Content, KeyRef, Move, Sampling, .count (+18 more)

### Community 15 - "CGPoint"
Cohesion: 0.05
Nodes (47): Int, CGPoint, .length, Basis, Corner, bottomLeft, bottomRight, topLeft (+39 more)

### Community 16 - "UIKit"
Cohesion: 0.04
Nodes (12): Combine, CoreGraphics, CoreText, Darwin, Metal, TextMeasure, PhotosUI, QuartzCore (+4 more)

### Community 17 - "KeyframeTarget"
Cohesion: 0.14
Nodes (22): KeyframePoseDelta, CanvasManager, .keyframeTarget, KeyframeControl, Keyframes, KeyframeState, KeyframeTarget, folder (+14 more)

### Community 18 - "VectorEraserLogicTests"
Cohesion: 0.09
Nodes (6): ClosedRange, StaticString, UInt, VectorStroke, VectorEraserLogicTests, .horizontalRun

### Community 19 - "LassoMoveLogicTests"
Cohesion: 0.07
Nodes (25): CodableColor, .uiColor, UIColor, .elements, VectorFillElement, .cgPath, .uiColor, LassoMoveLogicTests (+17 more)

### Community 20 - ".rasterize"
Cohesion: 0.09
Nodes (28): Hashable, StripWindow, CGSize, FrozenCel, Identity, IntPoint, PixelOps, .rasterizeCacheBytes (+20 more)

### Community 21 - ".activeCelIndex"
Cohesion: 0.11
Nodes (18): .activeLayerIsVector, .interpolationTarget, CanvasManager, Bool, Int, Range, Cel, .endFrame (+10 more)

### Community 22 - "CanvasManager"
Cohesion: 0.10
Nodes (22): CanvasManager, .fillEdgeOverlap, .fillHalfCoverageAlpha, FillGestureContext, FillKey, FillRenderResult, Bool, Cel (+14 more)

### Community 23 - "CodingKeys"
Cohesion: 0.06
Nodes (33): CodingKeys, alphaMask, animationGroups, backgroundColor, blendMode, canvasHeight, canvasPadding, canvasWidth (+25 more)

### Community 24 - "Effect"
Cohesion: 0.06
Nodes (19): Effect, .displayName, .input, .kind, .kindCode, .params, .readsAbsolutePosition, .reshapesCoverage (+11 more)

### Community 25 - ".drawLine"
Cohesion: 0.11
Nodes (13): FillContainmentUITests, FillLiveAdjustUITests, FillUndoRedoUITests, PaintUITestCase, Bool, CGVector, Double, TimeInterval (+5 more)

### Community 26 - "TextHitTestLogicTests"
Cohesion: 0.14
Nodes (8): Bool, CGAffineTransform, CGSize, String, UIScribbleInteraction, VectorElement, VectorStroke, TextHitTestLogicTests

### Community 27 - "CanvasManager"
Cohesion: 0.13
Nodes (17): CanvasManager, .layerStackRows, ContainerEntry, folder, layer, StackAnchor, bottom, folder (+9 more)

### Community 28 - "PerfBaselineTests"
Cohesion: 0.07
Nodes (19): CompositorBudget, .budgetOverrideBytes, .textureBudgetBytes, UInt64, PerfBaselineTests, Bool, BrushStamper, CanvasManager (+11 more)

### Community 29 - "Fill.metal"
Cohesion: 0.27
Nodes (28): device, colourDistance(), computeWalls(), distanceToCanvasEdge(), edgeBridge(), edgeDilate(), floodHoriz(), floodInit() (+20 more)

### Community 30 - "Lattice"
Cohesion: 0.06
Nodes (27): vertices, DeformedCellIndex, Hit, Lattice, .cellCount, .currentBounds, .restBounds, .restConfiguration (+19 more)

### Community 31 - "StrokeScratch"
Cohesion: 0.09
Nodes (18): Role, additive, replacing, StrokeScratch, .dirtyRect, .image, .replacesBase, .windowPixelCount (+10 more)

### Community 32 - "HistoryActionLabel"
Cohesion: 0.02
Nodes (81): HistoryActionLabel, addEffectLayer, addFolder, addFrame, addGuide, addKeyframe, addLayer, addMotionGroup (+73 more)

### Community 33 - "ActivePanel"
Cohesion: 0.11
Nodes (18): ActivePanel, actions, brush, color, eraser, fill, layers, move (+10 more)

### Community 34 - "Binding"
Cohesion: 0.06
Nodes (40): Accessory, KeyPath, .timelinePanel, BrushSettingsPanel, .body, .importCustomBrushRow, .preview, CanvasManager (+32 more)

### Community 35 - "VectorFloat"
Cohesion: 0.06
Nodes (28): CanvasManager, .fittedMoveBoxFrame, Cluster, MoveBoxInk, Bool, CGAffineTransform, CGPath, CGRect (+20 more)

### Community 36 - "FloatingPieceOverlayView"
Cohesion: 0.10
Nodes (19): FloatingTransform, FloatingPieceOverlayView, Bool, CGRect, CGSize, Int, NSCoder, UIPanGestureRecognizer (+11 more)

### Community 37 - "AnimationTimeline"
Cohesion: 0.05
Nodes (43): Expansion, Int, TimelineRowLayout, .contentHeight, .rowCount, AnimationTimeline, .collapsedBar, .contentHeight (+35 more)

### Community 38 - "View"
Cohesion: 0.13
Nodes (31): .layerPanelRail, blendModeRow(), effectSettingsRow(), FolderOptionsPanel, .body, .folder, .folderIndex, LayerOptionsPanel (+23 more)

### Community 39 - "ProjectSaveLogicTests"
Cohesion: 0.08
Nodes (18): MainActor, Void, wait, ProjectSaveLogicTests, Any, Bool, CanvasManager, Cel (+10 more)

### Community 40 - "LayerStackCell"
Cohesion: 0.11
Nodes (9): LayerStackCell, Bool, Double, Int, NSCoder, NSLayoutConstraint, UIView, Void (+1 more)

### Community 41 - "VectorCanvas"
Cohesion: 0.04
Nodes (67): CGPathFillRule, CachedRender, empty, .image, needsRasterize, ready, .version, elements (+59 more)

### Community 42 - "ColorPickerPanel"
Cohesion: 0.07
Nodes (35): Palette, PaletteColor, .color, PaletteStore, .palettes, .selectedIndex, .selectedPalette, .selectedPaletteID (+27 more)

### Community 43 - "SelectionOverlayView"
Cohesion: 0.06
Nodes (27): LassoFillDiagnostic, CGPath, Double, TimeInterval, UIImage, UUID, resolvedLastTouchType(), UITouch (+19 more)

### Community 44 - "VectorCanvasData"
Cohesion: 0.06
Nodes (34): Encoder, K, KeyedDecodingContainer, DecodeReport, .droppedCount, .isClean, ElementData, fill (+26 more)

### Community 45 - "FrameExportSession"
Cohesion: 0.13
Nodes (18): CheckedContinuation, task, FrameExportSession, .outputDirectory, Product, frame, video, Bool (+10 more)

### Community 46 - "ContentView"
Cohesion: 0.10
Nodes (17): App, AppScreen, editor, gallery, sizePicker, ContentView, .body, PendingDamagedSave (+9 more)

### Community 47 - "PerfMonitor"
Cohesion: 0.16
Nodes (13): CADisplayLink, PerfHUDOverlay, .body, .hudBody, .toggleButton, .totalCelCount, PerfMonitor, .isRunning (+5 more)

### Community 48 - "StrokeGeometryLogicTests"
Cohesion: 0.05
Nodes (16): MembershipRun, Capsule, .boundingBox, Intersection, StrokeGeometry, Bool, CGRect, ClosedRange (+8 more)

### Community 49 - "LayerManifest"
Cohesion: 0.08
Nodes (33): CodingKeys, kind, mixMode, op, CompositorRole, node, Decoder, Encoder (+25 more)

### Community 50 - "CanvasSizePickerView"
Cohesion: 0.14
Nodes (14): CanvasSizePickerView, .body, .height, .isValid, .width, Field, height, width (+6 more)

### Community 51 - "TextRecipe"
Cohesion: 0.08
Nodes (15): Mode, affine, projective, CodableColor, Decoder, Double, UUID, TextRecipe (+7 more)

### Community 52 - "TimelineKeyMarkersLogicTests"
Cohesion: 0.07
Nodes (17): Marker, Run, .isCollapsed, Bool, CGRect, ClosedRange, Int, Set (+9 more)

### Community 53 - "ObjectTransformFrame"
Cohesion: 0.10
Nodes (18): Handle, Handle, bottomLeft, bottomRight, boxRotation, .isCorner, .isDrawn, rotation (+10 more)

### Community 54 - "TextOverlayView"
Cohesion: 0.08
Nodes (24): RenderKey, Bool, CGPath, CGRect, CGSize, Float, NSCoder, Set (+16 more)

### Community 55 - "FrameExportLogicTests"
Cohesion: 0.11
Nodes (9): FrameExport, ClosedRange, UInt8, FrameExportLogicTests, CGImage, Double, Int, UInt8 (+1 more)

### Community 56 - "Housekeeping pass (8 items from 2026-07-22 feature audit)"
Cohesion: 0.14
Nodes (15): AppVersion.current stale hardcoded git hash, Color picker discarded hue on gray swatch, deleteCel no-op on layer's last remaining cel, Fill tool off-center fill vertically mirrored (regression), flipCanvas skipped object (photo) layers, FloodFillEngine.fill, ProjectStore.save composites every visible layer for thumbnail, testFillToolBridgesOpenContourGapWhenGapClosingEnabled (XCTSkip) (+7 more)

### Community 57 - "XCUIApplication"
Cohesion: 0.23
Nodes (9): CuttingModesUITests, Mode1UITests, ModePickerUITests, Int, String, XCUIApplication, XCUIElement, VectorEraserTestSupport (+1 more)

### Community 58 - "RenderRequest"
Cohesion: 0.10
Nodes (32): Cel, CanvasManager, ChunkContinuation, LayerContentVersion, LayerRenderSource, RenderBackground, RenderRequest, .effectOrigin (+24 more)

### Community 59 - "BrushEngineLogicTests"
Cohesion: 0.13
Nodes (12): BrushEngineLogicTests, Any, CodableColor, Double, Int, String, T, UIColor (+4 more)

### Community 60 - "AlphaMask"
Cohesion: 0.04
Nodes (44): CacheKey, MaskCache, .entryCount, MaskResolver, .cacheEntryCount, ResolvedMask, CGImage, Hasher (+36 more)

### Community 61 - "ProjectStore"
Cohesion: 0.10
Nodes (37): CFAbsoluteTime, CelContent, DecodedCel, DecodedCels, LayerContent, LoadProfile, .millisecondsPerCel, .thumbnailShare (+29 more)

### Community 62 - "FrameRecipe"
Cohesion: 0.05
Nodes (50): Content, cel, solid, FrameRecipe, LeafSnapshot, SandwichRecipe, .aboveRecipe, .belowRecipe (+42 more)

### Community 63 - "SideToolbar"
Cohesion: 0.16
Nodes (14): SideToolbar, .body, .eyedropperButton, .isEraserMode, .isFillMode, .sliderHeight, Bool, CanvasManager (+6 more)

### Community 64 - "CompositorParityLogicTests"
Cohesion: 0.07
Nodes (14): CompositorParityLogicTests, BlendMode, Bool, CanvasManager, CGImage, Int, MaskSource, RenderNode (+6 more)

### Community 65 - "CanvasManager"
Cohesion: 0.05
Nodes (44): Identifiable, CanvasManager, .activeCelIsInBetween, .guideChips, .guideRefusal, .hasAnonymousWholeFrameGroup, .interpolationCommitOptions, .interpolationContentProvider (+36 more)

### Community 66 - "EffectSettingsBar"
Cohesion: 0.08
Nodes (36): Layout, stops, CodableColor, .color, Color, .effectColor, ContentHeightCap, EffectCatalog (+28 more)

### Community 67 - ".assertChunkedMatchesWholeOnGPU"
Cohesion: 0.07
Nodes (19): UIColor, ChunkedCompositeMetalLogicTests, Bool, CanvasManager, Int, StaticString, String, UInt (+11 more)

### Community 68 - "GalleryOpenState"
Cohesion: 0.08
Nodes (25): os, CodableColor, .color, Color, .codable, ProjectSummary, CodableColor, Date (+17 more)

### Community 69 - "SaveDamageGateLogicTests"
Cohesion: 0.17
Nodes (9): SaveDamageGateLogicTests, Any, CanvasManager, StaticString, String, UInt, URL, VectorStroke (+1 more)

### Community 70 - "PaintSoftware iPad drawing/animation app"
Cohesion: 0.18
Nodes (13): Animation Timeline feature, Brush library feature (shape/hardness/spacing/stabilization/grain), Native-resolution raster/vector drawing engine (no PencilKit), GPU (Metal) colour-based flood fill feature, Gallery / project browser feature, Layers feature (raster/vector/object layers), PaintSoftware iPad drawing/animation app, Project directory structure (Engine/Models/Services/Utilities/Views) (+5 more)

### Community 72 - "PerfBaselineTests.swift"
Cohesion: 0.18
Nodes (11): CelCRUDCharacterizationTests, Shared frame-length clamp relaxed to >=, duplicateCel adjacent-neighbour overlap bug, Autorelease artifact in re-measuring memory (renderToUIImage), BrushStamper.stampStroke, Stroke cost tracks path length, not sample count, PerfBaselineTests.swift, Refactor Stage 0 — baseline + characterization tests (+3 more)

### Community 73 - "LayerTreeCharacterizationTests"
Cohesion: 0.05
Nodes (15): Layer, StaticString, String, UInt, UUID, LayerTreeCharacterizationTests, CanvasManager, String (+7 more)

### Community 74 - "ColorMathOklabLogicTests"
Cohesion: 0.06
Nodes (42): Color, .hexString, .hsbaComponents, .rgbaComponents, Double, String, UIColor, ColorMath (+34 more)

### Community 75 - "ObjectTransformOverlayView"
Cohesion: 0.09
Nodes (18): ObjectTransformOverlayView, .canvasScale, .drawnChrome, .handleBorderWidth, .handleReach, .handleSize, .outlineWidth, .rotationHandleSize (+10 more)

### Community 76 - "CompositorMetalEngine"
Cohesion: 0.08
Nodes (36): Attempt, image, unavailable, underPressure, BlendMode, .shaderCode, CompositorMetalEngine, .residentPoolHighWaterBytes (+28 more)

### Community 77 - "KeyframeControlLogicTests"
Cohesion: 0.14
Nodes (5): KeyframeControlLogicTests, CanvasManager, Double, Int, String

### Community 78 - "The layer transform — keep it, or bake it into the geometry?"
Cohesion: 0.09
Nodes (21): 0. The ruling, and what this document is answering, 10. Would a *translation-only* transform be safe? — asked 2026-08-28, answered no, 1. What `_transform` actually buys today, 1a. The census — every read and every write, 1b. Per feature — does it need a layer transform, or a way to map a gesture into storage?, 2. What breaks if it is removed, in order of severity, 3. Migration, 4. The live drag, and the question that decides its cost (+13 more)

### Community 79 - "CodingKeys"
Cohesion: 0.05
Nodes (47): CodingKeys, amount, angleDegrees, brightness, color, contrast, gamma, hueDegrees (+39 more)

### Community 80 - ".makeUIView"
Cohesion: 0.09
Nodes (11): CanvasView, OnionSkinKey, CGAffineTransform, CGImage, CGSize, Context, Coordinator, ObjectIdentifier (+3 more)

### Community 81 - "Equatable"
Cohesion: 0.12
Nodes (27): Equatable, Bloom, Blur, BrightnessContrast, ChromaticAberration, value, Curves, levels (+19 more)

### Community 82 - "SandwichLogicTests"
Cohesion: 0.08
Nodes (8): Battery, SandwichLogicTests, BlendMode, CanvasManager, CGImage, Int, RenderNode, String

### Community 83 - "CGImage.cropping(to:) retains parent pixel data"
Cohesion: 0.50
Nodes (4): CGImage.cropping(to:) retains parent pixel data, PixelOps.copiedSubimage, UIGraphicsImageRendererFormat.preferredRange defaults to extended-range on wide-colour iPad, Stage 5 performance work (dab gradient cache, dirty-rect undo)

### Community 84 - "Multi-Session Protocol"
Cohesion: 0.33
Nodes (6): Deploy to iPad (deploy.sh, auto-resign), Multi-Session Protocol, deploy/mac/parallel_test.sh, Remote testing via Tailscale to Mac, Worktree-per-session workflow, Foolproof project backups (Session 34)

### Community 85 - "graphify usage protocol"
Cohesion: 0.67
Nodes (3): graphify-out/GRAPH_REPORT.md, .claude/hooks/graphify-guard.sh, graphify usage protocol

### Community 86 - "VectorCanvasDataLogicTests"
Cohesion: 0.15
Nodes (11): Any, Double, Int, String, T, UIColor, UIImage, VectorElement (+3 more)

### Community 87 - "TextBakeCharacterizationTests"
Cohesion: 0.16
Nodes (7): CanvasManager, CGRect, Int, String, UInt8, VectorElement, TextBakeCharacterizationTests

### Community 88 - "InterpolationMotionGroupLogicTests"
Cohesion: 0.12
Nodes (10): InterpolationMotionGroupLogicTests, .twoBodiesAtRest, .twoBodiesMoved, CanvasManager, Cel, CodableColor, Int, UUID (+2 more)

### Community 89 - "parallel_test.sh"
Cohesion: 0.70
Nodes (4): claim_sim(), log(), release_sim(), parallel_test.sh script

### Community 90 - ".solidImage"
Cohesion: 0.08
Nodes (13): UIImage, EffectLayerLogicTests, .side, CanvasManager, CGImage, Double, Int, RenderNode (+5 more)

### Community 91 - "DeformFactorization"
Cohesion: 0.11
Nodes (14): Accelerate, DeformDataRow, DeformEdgeTerm, DeformFactorization, Matrix2x2, .determinant, .isFinite, .polar (+6 more)

### Community 92 - "EffectMultiPassLogicTests"
Cohesion: 0.12
Nodes (6): EffectMultiPassLogicTests, Double, Int, SIMD4, String, UInt8

### Community 93 - ".evaluate"
Cohesion: 0.12
Nodes (25): CGPathElementType, ContentProvider, Direction, backward, forward, fromRest, Evaluation, GroupWarp (+17 more)

### Community 94 - "InterpolationRenderLogicTests"
Cohesion: 0.17
Nodes (11): ID, InterpolationRenderLogicTests, CGSize, CodableColor, Int, InterpolationRecipe, RenderQuality, UIImage (+3 more)

### Community 95 - "Layer Compositing"
Cohesion: 0.07
Nodes (29): 10. Still open, 11. Build order, 1. Why these are one project, 2. What is *not* changing, 3. Settled decisions, 4.1 Structure, 4.2 Isolated groups, 4.3 Compositor nodes (+21 more)

### Community 109 - "BlendMode"
Cohesion: 0.06
Nodes (31): BlendMode, add, clipToBelow, color, colorBurn, colorDodge, .compositedMode, darken (+23 more)

### Community 110 - "ObjectTransformDrag"
Cohesion: 0.15
Nodes (5): .placement, ObjectTransformDrag, .corners, Pose, CGSize

### Community 111 - "GuideStroke"
Cohesion: 0.08
Nodes (26): InterpolationRefusal, alreadyInterpolated, interpolationNotEvaluable, .message, noInterpolationToGuide, notAVectorLayer, notEnoughReferences, nothingToCommit (+18 more)

### Community 112 - "float3"
Cohesion: 0.22
Nodes (24): float3, blendChannels(), blendColor(), blendColorBurn(), blendColorDodge(), blendDarkerColor(), blendDivide(), blendExclusion() (+16 more)

### Community 113 - "PlaybackBoundsCharacterizationTests"
Cohesion: 0.08
Nodes (9): PlaybackClock, Int, TimeInterval, FakeClock, PlaybackBoundsCharacterizationTests, Bool, CanvasManager, Int (+1 more)

### Community 114 - "GuideOverlayView"
Cohesion: 0.12
Nodes (17): Editing, handles, none, spacing, Grip, Guide, GuideOverlayView, Bool (+9 more)

### Community 115 - "BrushBlendMode"
Cohesion: 0.07
Nodes (26): BrushBlendMode, .cgBlendMode, darken, .id, lighten, multiply, normal, screen (+18 more)

### Community 116 - "LassoFillLogicTests"
Cohesion: 0.19
Nodes (6): LassoFillLogicTests, .loopAroundEverything, CanvasManager, CGRect, TimeInterval, UIImage

### Community 117 - "GuidePath"
Cohesion: 0.14
Nodes (9): GuidePath, GuideSet, .isEmpty, Bool, CGVector, InterpolationRecipe, TimeInterval, InterpolationRecipe (+1 more)

### Community 118 - "EffectParityLogicTests"
Cohesion: 0.13
Nodes (4): EffectParityLogicTests, Int, String, UInt8

### Community 119 - "AnimationCurve"
Cohesion: 0.09
Nodes (31): AnimationCurve, .isAnimated, .isEmpty, CodingKeys, frame, inHandle, interpolation, keys (+23 more)

### Community 120 - ".group"
Cohesion: 0.16
Nodes (5): Group, MotionGrouping, Options, Int, Set

### Community 121 - ".apply"
Cohesion: 0.28
Nodes (10): EffectParams, EffectReference, CGImage, Float, Int, SIMD2, SIMD4, UInt32 (+2 more)

### Community 122 - "VectorSample"
Cohesion: 0.11
Nodes (11): VectorSample, .point, CutPreviewEdit, Restamp, ClosedRange, CountingDabTarget, StrokeSampleGateLogicTests, CGBlendMode (+3 more)

### Community 123 - "InterpolationGuideLogicTests"
Cohesion: 0.07
Nodes (10): GuideHandles, Int, TimeInterval, TimedSample, .point, InterpolationGuideLogicTests, CanvasManager, Cel (+2 more)

### Community 124 - "TextTransformOverlayView"
Cohesion: 0.10
Nodes (21): TextCornerMode, .displayName, distort, .id, scale, Bool, CALayer, CGRect (+13 more)

### Community 125 - "command"
Cohesion: 0.29
Nodes (7): command, deploy, resign, description, template, description, template

### Community 126 - "Compositor.swift"
Cohesion: 0.17
Nodes (20): BlendMode, .coreGraphicsBlendMode, .isNonSeparable, clipColor(), colorBurnChannel(), colorDodgeChannel(), Compositor, .backend (+12 more)

### Community 127 - "FontDescriptor"
Cohesion: 0.12
Nodes (20): FontFace, .descriptor, .id, FontFamilyGroup, .id, FontLibrary, FontProvider, FontResolution (+12 more)

### Community 128 - "CanvasManager"
Cohesion: 0.18
Nodes (11): CanvasManager, CelDropRequest, CelDropVerdict, allowed, needsRasterization, rejected, Bool, Cel (+3 more)

### Community 129 - "linear_light_q1q2.swift"
Cohesion: 0.07
Nodes (62): blend6(), blendOver6(), box(), Buf, .cgImage, clipColor(), darkDab(), enc3() (+54 more)

### Community 130 - "Typography"
Cohesion: 0.15
Nodes (11): ClosedRange, Typography, .clamped, CGRect, CGSize, ClosedRange, Int, String (+3 more)

### Community 131 - "Composite.metal"
Cohesion: 0.25
Nodes (32): float2, read, applyEffect(), blendOver(), bloomCombine(), bloomThreshold(), blur1D(), chromaticAberration() (+24 more)

### Community 132 - "EyedropperLogicTests"
Cohesion: 0.11
Nodes (8): Eyedropper, Sample, CGSize, Double, Int, UInt8, EyedropperLogicTests, UInt8

### Community 133 - "Codable"
Cohesion: 0.06
Nodes (30): Codable, CelRef, InterpolationMode, generate, reproject, InterpolationRecipe, .isWellFormed, .referencedCels (+22 more)

### Community 134 - "GuideRow"
Cohesion: 0.12
Nodes (16): GuideRow, .averagingNote, .body, .guideButton, .hasASpacingChart, .hasRecipe, .spacingButton, Bool (+8 more)

### Community 135 - "InterpolateBar"
Cohesion: 0.12
Nodes (17): .body, InterpolateBar, .activeRecipe, .commandRow, .commands, .commitButton, .referenceButton, .referenceSummary (+9 more)

### Community 136 - "XCUIApplication"
Cohesion: 0.08
Nodes (19): BlendModesAndCompositorUITests, LayerFolderAndMaskMenuUITests, LayerPanelControlsUITests, LayerStackUITests, SandwichCompositingUITests, .crossing, Bool, CGVector (+11 more)

### Community 137 - "EffectParams"
Cohesion: 0.09
Nodes (22): EffectParams, amount, brightness, colorB, colorG, colorR, contrast, hueTurns (+14 more)

### Community 138 - "VectorEraserHybridLogicTests"
Cohesion: 0.06
Nodes (49): UIImage, UInt8, Backdrop, fill, image, none, Gesture, diagonalCut (+41 more)

### Community 139 - "BakeKeyEncoder"
Cohesion: 0.06
Nodes (35): CryptoKit, BakeKeyEncodable, BakeKeyEncoder, BlendMode, .bakeKeyTag, CompositorBackend, .bakeKeyTag, MaskSource (+27 more)

### Community 140 - "CanvasResizeLogicTests"
Cohesion: 0.07
Nodes (18): CanvasResizeMap, .contentRect, .inverse, .losesRasterFidelity, .transform, CGAffineTransform, CGRect, CanvasResizeLogicTests (+10 more)

### Community 141 - ".launchIntoEditor"
Cohesion: 0.13
Nodes (9): EraserAndPersistenceUITests, SelectionAndMoveUITests, Double, Int, StaticString, UInt, XCUIApplication, XCUIElement (+1 more)

### Community 142 - "CanvasNotice"
Cohesion: 0.06
Nodes (22): Kind, CanvasNotice, .actionTitle, .code, .duration, .message, Kind, hiddenLayer (+14 more)

### Community 143 - ".assertLinear"
Cohesion: 0.10
Nodes (10): Decomposition, .aspect, .scale, LiveLayerTransform, CGAffineTransform, CGAffineTransform, StaticString, String (+2 more)

### Community 144 - "OnionSkinSettingsSource"
Cohesion: 0.11
Nodes (13): Colouring, .id, originalColors, tinted, .title, OnionSkinSettingsSource, OnionSkinSource, Bool (+5 more)

### Community 145 - "4. Future upgrades — the deferred list"
Cohesion: 0.33
Nodes (6): 4. Future upgrades — the deferred list, Explicitly deferred — do not build without being asked, Model and correctness gaps, Performance, The big one — engine upgrade candidates, UI and workflow

### Community 146 - "CaseIterable"
Cohesion: 0.08
Nodes (26): CaseIterable, Input, backdrop, FillMode, .displayName, flood, .id, lasso (+18 more)

### Community 147 - "Kind"
Cohesion: 0.14
Nodes (14): Kind, bloom, blur, brightnessContrast, chromaticAberration, curves, gradientMap, hsvShift (+6 more)

### Community 148 - "FillGestureRestartLogicTests"
Cohesion: 0.22
Nodes (7): FillGestureRestartLogicTests, CanvasManager, CGPath, CGRect, Int, TimeInterval, UInt8

### Community 149 - "LayerStackListView"
Cohesion: 0.18
Nodes (9): IndexPath, .body, LayerStackListView, Context, Coordinator, Void, UISwipeActionsConfiguration, UITableView (+1 more)

### Community 150 - "CanvasManager"
Cohesion: 0.06
Nodes (42): CanvasManager, .canResetFloating, .displayedSelectionMembership, .distortUnavailableReason, .isAnyPieceFloating, .recolorUnavailableReason, .selectionMembershipUnavailableReason, .vectorFloatIsFreeform (+34 more)

### Community 151 - "PinchMergeGateLogicTests"
Cohesion: 0.14
Nodes (7): PinchMergeGate, RowLayout, Bool, Int, PinchMergeGateLogicTests, Bool, Int

### Community 152 - "ExportSheet"
Cohesion: 0.13
Nodes (14): ExportSheet, .body, .caption, .exportSize, .running, .statusLine, .videoDetail, .videoRange (+6 more)

### Community 153 - ".rasterize"
Cohesion: 0.17
Nodes (6): LassoFillMask, Float, Int, SIMD4, UInt8, mask

### Community 154 - "RasterLayerTexture"
Cohesion: 0.14
Nodes (14): RasterLayerTexture, .approximateCost, .dabGradientCacheHits, .dabGradientCacheMisses, .hasContent, .strokeDirtyRect, Bool, CGRect (+6 more)

### Community 155 - "Gesture"
Cohesion: 0.13
Nodes (14): build_gestures(), element_expression(), emit_gesture(), emit_state_comments(), Gesture, main(), normalized_expression(), parse() (+6 more)

### Community 156 - "CanvasTransformFreezeUITests"
Cohesion: 0.21
Nodes (7): CanvasTransformFreezeUITests, Bool, Int, String, XCUIApplication, XCUIElement, XCUICoordinate

### Community 157 - "ShapeHoldClock"
Cohesion: 0.19
Nodes (9): ShapeHoldClock, .isHoldComplete, .stillDuration, Bool, TimeInterval, ShapeHoldClockLogicTests, Bool, Int (+1 more)

### Community 159 - "CanvasTouchInputs"
Cohesion: 0.09
Nodes (19): CanvasTouchInputs, .activeHostIsInteractive, .activeHostReceivesTouches, .catchAllIsEnabled, .catchAllRaisesNotice, .eyedropperPressIsEnabled, .fillPressIsEnabled, .floatingOverlayIsInteractive (+11 more)

### Community 160 - "Keyframes"
Cohesion: 0.07
Nodes (30): 10. Traps inherited from elsewhere, 11.1 It lives inside the timeline's scroll content, 11.2 Stage D1 — variable row height, behaviour-neutral, merged alone — **done 2026-08-29**, 11.3 Stage D2 — the band, and three ways it renders once and never again — **done 2026-08-29**, 11.4 Stage D3 — the gestures — **done 2026-08-29, both halves**, 11.5 Stage D4 — the channel list is a filter, not a navigator — **done 2026-08-29**, 11.6 Open, 11. The graph editor (+22 more)

### Community 161 - "CanvasManager"
Cohesion: 0.16
Nodes (6): CanvasManager, .activeEditColor, .isTextInAdjustableState, Bool, Color, String

### Community 162 - ".lassoFill"
Cohesion: 0.18
Nodes (4): ink, Bool, Double, Float

### Community 163 - "CurveEditor"
Cohesion: 0.21
Nodes (11): CurvePoint, MonotoneCubic, CurveEditor, .body, .curvePath, .dragGesture, .grid, .handles (+3 more)

### Community 164 - "OnionSkinPanel"
Cohesion: 0.11
Nodes (21): .onionSkinButton, CodableColor, .swiftUIColor, OnionSkinPanel, .body, .caution, .colouringPicker, .countRow (+13 more)

### Community 165 - "EffectParameter"
Cohesion: 0.13
Nodes (30): AnyKeyPath, O, P, .parameters, EffectCaseLens, EffectParameter, .isScalarAnimatable, EffectParameterAnimation (+22 more)

### Community 166 - "Recording"
Cohesion: 0.13
Nodes (14): Recording, .id, .name, .sizeText, Date, Int, ActionRecorderIndicator, .body (+6 more)

### Community 167 - "StrokeSpatialIndex"
Cohesion: 0.13
Nodes (12): Int64, SegmentRef, StrokeSpatialIndex, .count, .isEmpty, Bool, CGRect, Int (+4 more)

### Community 168 - "RenderNode"
Cohesion: 0.07
Nodes (35): Array, Arity, fixed, variadic, Array, .containsAGrade, .effectIntermediateTextures, .gpuLeafThreshold (+27 more)

### Community 169 - "BakeQueueLogicTests"
Cohesion: 0.09
Nodes (15): BakeQueue, .frameCount, .pendingCount, Direction, backward, forward, Bool, ClosedRange (+7 more)

### Community 170 - "VectorPreviewPlanLogicTests"
Cohesion: 0.11
Nodes (14): Base, committedRender, interpolation, Bool, String, VectorPreviewPlan, VectorScratchRole, none (+6 more)

### Community 171 - "1. The decisions"
Cohesion: 0.11
Nodes (18): 1. The decisions, 2. Why the rejected alternatives were rejected, 3. Staged delivery, 4. Performance rules, 5. Behaviour, decided, `ActionsMenu` gains the ability to enter a mode, Add Text, Fonts go through one seam and nothing else (+10 more)

### Community 172 - "EffectParameterTrackLogicTests"
Cohesion: 0.15
Nodes (5): EffectParameterTrackLogicTests, CanvasManager, Double, Int, UUID

### Community 173 - "Int"
Cohesion: 0.22
Nodes (6): OnionSkinBudget, OnionSkinOpacityRamp, CGSize, Double, Int, Int

### Community 174 - "TextLayout"
Cohesion: 0.18
Nodes (14): CTFrame, CTFramesetter, NSAttributedString, NSRange, NSTextAlignment, Line, Metrics, Bool (+6 more)

### Community 175 - "CodingKeys"
Cohesion: 0.08
Nodes (26): CodingKeys, alignment, animationGroupID, autoSize, color, corners, faceName, familyName (+18 more)

### Community 176 - ".setUpGestures"
Cohesion: 0.11
Nodes (13): .isLassoFilling, Set, UIEvent, UIPanGestureRecognizer, UIPinchGestureRecognizer, UITapGestureRecognizer, UITouch, UIView (+5 more)

### Community 177 - ".compositeSize"
Cohesion: 0.12
Nodes (13): NSObjectProtocol, .resolutionNoteText, Key, OnionSkinFrame, OnionSkinRasterCache, .residentBytes, AnyHashable, CanvasManager (+5 more)

### Community 178 - "MetalFillSession"
Cohesion: 0.19
Nodes (16): MTLBuffer, MTLCommandBuffer, FillParams, MetalFillEngine, .pipelines, MetalFillSession, Bool, Float (+8 more)

### Community 179 - "OnionSkinSettings"
Cohesion: 0.17
Nodes (10): .opacitySliders, OnionSkinSettings, Side, .id, next, .step, Bool, CodableColor (+2 more)

### Community 180 - "0. What already exists"
Cohesion: 0.14
Nodes (14): 0. What already exists, 1. Every tier, and what a resize owes it, 3. Why the rejected alternatives were rejected, 4. Staged delivery, 5. Behaviour, decided, 6. Open questions for the owner, Canvas Resize — Specification, `CanvasSizePickerView` is create-only and is not on this path (+6 more)

### Community 181 - "TODO.md"
Cohesion: 0.19
Nodes (6): 1. What it does, 2. Architecture, 3. Settled decisions and hard-won facts, 5. Open judgement calls for the product owner, Vector Interpolation, What the papers say

### Community 182 - "Is the brush engine ready for `.ABR` / Procreate brush import?"
Cohesion: 0.14
Nodes (14): 1. One stamper serves every tier, 2. The replayability contract, 3. Imported-asset lifetime is already solved, 4. The eraser degrades gracefully by construction, `Brush` will need grouping before it needs fields, `BrushShape` and `customTextureFileName` should become one thing, Is the brush engine ready for `.ABR` / Procreate brush import?, Order, if it is ever scheduled (+6 more)

### Community 183 - "TransformChannelID"
Cohesion: 0.10
Nodes (27): CanvasManager, CelPoseState, .isEmpty, PosedCelIdentity, Bool, Cel, CGAffineTransform, Int (+19 more)

### Community 184 - "WarpAgreementCharacterizationTests"
Cohesion: 0.09
Nodes (21): ImageWarp, MetalWarpEngine, CGImage, CGRect, CGSize, Float, Int, MTLCommandQueue (+13 more)

### Community 185 - "AnimationCurveLogicTests"
Cohesion: 0.08
Nodes (11): Curve, Float, AnimationCurveLogicTests, Double, Int, Key, MaskGuardLogicTests, ClosedRange (+3 more)

### Community 186 - "simlock.sh"
Cohesion: 0.83
Nodes (3): reap_stale(), release(), simlock.sh script

### Community 187 - "CanvasTouchOwnerLogicTests"
Cohesion: 0.17
Nodes (3): CanvasTouchOwnerLogicTests, String, Void

### Community 188 - ".rgbaBytes"
Cohesion: 0.05
Nodes (18): CompositeProbe, CGSize, CGImage, CGRect, UInt8, CGSize, FrameRecipeLogicTests, CanvasManager (+10 more)

### Community 189 - "EffectPipelines"
Cohesion: 0.16
Nodes (14): MTLLibrary, EffectPipelines, .residentIntermediateSizes, MetalEffectEngine, Bool, Int, MTLCommandQueue, MTLComputeCommandEncoder (+6 more)

### Community 191 - "FillBoundaryLogicTests"
Cohesion: 0.19
Nodes (6): FillBoundaryLogicTests, Bool, ClosedRange, Float, Int, UInt8

### Community 192 - "CanvasResizeSheet"
Cohesion: 0.07
Nodes (34): ActionsMenu, .addTextRow, .bakePrecisionRow, .body, .content, .exportRow, .paddingControl, .pencilOnlyToggle (+26 more)

### Community 193 - "TextSettingsPanel"
Cohesion: 0.15
Nodes (15): CanvasManager, ClosedRange, Double, String, WritableKeyPath, TextSettingsPanel, .alignmentBinding, .alignmentPicker (+7 more)

### Community 194 - "String"
Cohesion: 0.08
Nodes (25): SizePreviewGeometry, .isClipped, .stampDiameter, .windowSide, SizePreviewRequest, SizePreviewSide, above, leading (+17 more)

### Community 195 - "UndoHistory"
Cohesion: 0.21
Nodes (9): Action, Bool, Int, Void, UndoBudget, UndoHistory, .canRedo, .canUndo (+1 more)

### Community 196 - "Performance"
Cohesion: 0.07
Nodes (27): 10.1 Compression ratio — MEASURED, exact, build-independent, 10.2 Encode and decode — MEASURED on the owner's iPad 9 in **Release**, 10.3 §3.5's contingency, evaluated before anyone builds it: a per-row filter **loses**, 10.4 What is still owed, 10. What a baked frame costs on disk and on the clock (2026-09-02), 1. The recalibration, and what it promotes, 2. What the artist actually feels, 3. The programme (+19 more)

### Community 197 - "PlacedImageShapeLogicTests"
Cohesion: 0.17
Nodes (10): PlacedImageShapeLogicTests, Bool, CanvasManager, CGAffineTransform, CGRect, Int, StaticString, String (+2 more)

### Community 198 - "UInt8"
Cohesion: 0.15
Nodes (5): CGPath, ClosedRange, Int, UInt64, UInt8

### Community 199 - "TimelineLayoutKeyLogicTests"
Cohesion: 0.11
Nodes (5): CGRect, Range, CanvasManager, Int, TimelineLayoutKeyLogicTests

### Community 200 - "CanvasHostView"
Cohesion: 0.15
Nodes (9): CanvasHostView, .accessibilityValue, .canBecomeFirstResponder, .keyCommands, Bool, CanvasManager, String, Void (+1 more)

### Community 201 - "Lasso Fill — Specification"
Cohesion: 0.15
Nodes (12): 1. The algorithm, in standard terms, 2. Is the owner's proposal right?, 2a. Where a fill lands in the stack: on top of everything already on the layer, 3. The rule, for an artist, 4. Edge cases, decided, 5. What shipped applications do, and where this diverges, 6. Pixel-level specification, 7. When there is nothing to fill (+4 more)

### Community 202 - "InterpolationEngineDiagnosticsLogicTests"
Cohesion: 0.27
Nodes (3): InterpolationEngineDiagnosticsLogicTests, Registration, Int

### Community 203 - "VideoFrameWriter"
Cohesion: 0.23
Nodes (14): AVAssetWriter, AVAssetWriterInput, AVAssetWriterInputPixelBufferAdaptor, Failure, couldNotAppend, couldNotFinish, couldNotStart, noPixelBuffer (+6 more)

### Community 204 - "TimelineRowLayoutLogicTests"
Cohesion: 0.13
Nodes (3): .rowLayout, Int, TimelineRowLayoutLogicTests

### Community 205 - "CelContentProviderLogicTests"
Cohesion: 0.15
Nodes (9): CelContentProviderLogicTests, .size, Bool, CanvasManager, Cel, CGSize, UIImage, UUID (+1 more)

### Community 206 - "XCTestCase"
Cohesion: 0.07
Nodes (8): XCTestCase, CanvasPresentationLogicTests, Bool, String, URL, Bool, Tool, ToolLogicTests

### Community 207 - "Layer"
Cohesion: 0.12
Nodes (14): Layer, .hasNoDrawingSurface, .isFillReference, .layerEffect, .valueFill, BlendMode, Bool, Cel (+6 more)

### Community 208 - "SizePreviewWindow"
Cohesion: 0.12
Nodes (17): Anchor, ObservableObject, .sizePreview, .uploadableLeafCount, CanvasDisplayScale, SizePreviewAnchorKey, SizePreviewWindow, .body (+9 more)

### Community 209 - "linear_light_ab.swift"
Cohesion: 0.13
Nodes (34): blendChannels(), blendOver(), Buf, .cgImage, composite(), context(), crop(), differenceMap() (+26 more)

### Community 210 - "CanvasTouchOwner"
Cohesion: 0.08
Nodes (26): CanvasTouchChrome, guideGrip, none, .owner, shapeHandleOrOutline, textBoxOrBand, textHandle, transformBoxOrHandle (+18 more)

### Community 211 - "CanvasPresentation"
Cohesion: 0.11
Nodes (15): CanvasPresentation, canvasBackgroundColour, effectGradientStopColour, effectOutlineColour, galleryProjectVersions, galleryRecentlyDeleted, graphChannelList, .id (+7 more)

### Community 212 - "MenuInterruptionUITests"
Cohesion: 0.33
Nodes (5): MenuInterruptionUITests, Int, String, XCUIApplication, XCUIElement

### Community 213 - "BlockDragCharacterizationTests"
Cohesion: 0.20
Nodes (4): BlockDragCharacterizationTests, Bool, CanvasManager, Int

### Community 214 - "Effect backdrop — what an adjustment layer grades"
Cohesion: 0.15
Nodes (12): §0 — What is already true, verified rather than assumed, §1 — The ruling, 2.1 Once an effect grades the paper, the composite is opaque from that node up, 2.2 Once the paper is in the accumulator, alpha stops meaning coverage, §2 — The two consequences are inherent, not incidental, §3 — Options for the ink-only input, with costs, §4 — What the effect declares, §5 — Answered, 2026-08-27 (+4 more)

### Community 215 - "LayerStackRow"
Cohesion: 0.12
Nodes (15): FolderKind, compositorNode, group, LayerStackRow, .depth, folder, .folderID, .folderKind (+7 more)

### Community 216 - "TimelineGraphChannelListLogicTests"
Cohesion: 0.19
Nodes (5): CanvasManager, Double, Int, String, TimelineGraphChannelListLogicTests

### Community 217 - "SpacingChart"
Cohesion: 0.21
Nodes (4): SpacingChart, .curve, .draggable, Range

### Community 218 - "Every dismissible presentation, and whether a stroke under it breaks"
Cohesion: 0.17
Nodes (11): BROKEN — a stroke under it dismisses it mid-sequence, nothing clears it first, Counts, Coverage limits of this sweep, Every dismissible presentation, and whether a stroke under it breaks, SAFE, and worth knowing why, SETTLED SAFE (was UNKNOWN) — presents through a path this repo had never verified, The contract, and why it did not hold, The four distinct versions of the problem (+3 more)

### Community 219 - "Group"
Cohesion: 0.17
Nodes (15): .graphChannelList, CanvasManager, .graphBandHasHiddenChannels, .graphChannelGroups, Filter, Group, .isFullyVisible, .isMixed (+7 more)

### Community 220 - "TransformTrack"
Cohesion: 0.08
Nodes (25): CelAnimationData, CodingKeys, baselines, frame, inHandle, interpolation, keys, outHandle (+17 more)

### Community 221 - "Brush"
Cohesion: 0.12
Nodes (15): Brush, CutOutcome, cut, missed, unchanged, IntersectionDriver, Sweep, Bool (+7 more)

### Community 222 - ".handleReorderDrag"
Cohesion: 0.23
Nodes (5): DispatchWorkItem, UILongPressGestureRecognizer, UIPinchGestureRecognizer, .gradientStops, previous

### Community 223 - "Open"
Cohesion: 0.09
Nodes (23): (10) Linear light as an option on the blend mode — deprioritised by the owner, (12) Lasso move — the follow-ups its spec still lists as unbuilt, (21) Keyframes — animating properties across the frames one cel spans, (22) Select multiple cels at once — the half of ask 6 the owner put in the future, (24) The canvas paper is its own code path — should it just be a value layer?, (26) Import videos — requires (21), (27) Screen recording the computer as a layer — requires (26), (28) Audio — needs a real playback clock first (+15 more)

### Community 224 - "CGFloat"
Cohesion: 0.08
Nodes (16): bendRatio(), cellSize(), coverage(), cShape(), polyline(), Int, Int16, CGFloat (+8 more)

### Community 225 - "TimelineLayoutKey"
Cohesion: 0.24
Nodes (13): CelKey, DragKey, FolderKey, Bool, CanvasManager, ClosedRange, Int, ObjectIdentifier (+5 more)

### Community 226 - "DecodedFrameRing"
Cohesion: 0.11
Nodes (14): DecodedFrame, .byteCount, DecodedFrameRing, .byteBudget, .byteCount, .count, Bool, CGImage (+6 more)

### Community 227 - "1. The decisions"
Cohesion: 0.08
Nodes (26): 0. What shipped, and where it lives, 1. The decisions, 2. Why the rejected alternatives were rejected, 3. Staged delivery, 4. Performance rules, 5. Behaviour, decided, 6. Open risks, A fill is a `CGPath`, and cutting a chunk out of it is two Core Graphics calls (+18 more)

### Community 229 - "Foundation"
Cohesion: 0.10
Nodes (8): AVFoundation, Foundation, ImageIO, CodingUserInfoKey, AppVersion, .versionString, String, UniformTypeIdentifiers

### Community 230 - "FrameBaker"
Cohesion: 0.09
Nodes (28): CelStamp, .span, ExportFocus, FrameBaker, FrameObserver, Outcome, alreadyOnDisk, compositeFailed (+20 more)

### Community 231 - "FrameBakerLogicTests"
Cohesion: 0.22
Nodes (6): FrameBakerLogicTests, CanvasManager, Int, TimeInterval, URL, Void

### Community 232 - "FrameBakeStoreLogicTests"
Cohesion: 0.18
Nodes (9): FrameBakeKey, .fileName, Bool, FrameBakeStoreLogicTests, CGImage, Int, UInt8, URL (+1 more)

### Community 233 - "WarpParams"
Cohesion: 0.20
Nodes (10): WarpParams, m0, m1, m2, m3, m4, m5, m6 (+2 more)

### Community 234 - ".previewed"
Cohesion: 0.36
Nodes (5): Double, Int, UIImage, VectorStroke, VectorCutPreviewLogicTests

### Community 235 - "OnionSkinLogicTests"
Cohesion: 0.20
Nodes (4): CelSpan, .end, OnionSkinPlanner, OnionSkinLogicTests

### Community 236 - "CodingKeys"
Cohesion: 0.15
Nodes (13): CodingKeys, boxH, boxW, boxX, boxY, x0, x1, x2 (+5 more)

### Community 237 - "TimelineBakeBar"
Cohesion: 0.11
Nodes (13): RefreshThrottle, .isPending, Span, .frameCount, Bool, CGRect, Int, String (+5 more)

### Community 238 - "1. What will actually hurt, ranked"
Cohesion: 0.20
Nodes (9): 1. Nothing answers "who owns this canvas touch" — and this is where the bugs are — CLOSED, `38b6fed`, 1. What will actually hurt, ranked, 2. What a frame looks like is memoized in eleven hand-written keys, 2. What is genuinely good — do not disturb, 3. A save that fails tells nobody, and one nil PNG fails the whole document, 3. What is not worth doing, 4. One persisted property means four hand-kept structs, and the initializer defaults hide the miss, 4. The owner's question, answered (+1 more)

### Community 239 - "SelectPanel"
Cohesion: 0.09
Nodes (22): .bottomDock, MoveTransformBottomBar, .body, .caption, .divider, .precisionToggle, Bool, CanvasManager (+14 more)

### Community 240 - "ARAPLogicTests"
Cohesion: 0.12
Nodes (10): ARAPInterpolation, Interpolator, Options, Bool, ARAPLogicTests, .rigidMotionL, Int, StaticString (+2 more)

### Community 241 - "PoseQuad"
Cohesion: 0.14
Nodes (11): PoseInterpolation, PoseQuad, Bool, CGAffineTransform, CGRect, CGVector, Decoder, Encoder (+3 more)

### Community 242 - "MemoryBudgetLogicTests"
Cohesion: 0.16
Nodes (6): UInt64, .maxCostBytes, MemoryBudgetLogicTests, Int, String, UInt64

### Community 243 - "3. Design"
Cohesion: 0.11
Nodes (19): 0. What is already true, 1. The ask, in the owner's words, 2. Rulings, 3.10 What a composite is not, 3.1 What the main thread does, and the pen-up cost this design removed, 3.2 The recipe — how pixel work leaves the main actor without a race, 3.3 The bake key, 3.4 Chunked compositing — the owner's method, shaped to the tree (+11 more)

### Community 244 - ".assertChunkedMatchesWhole"
Cohesion: 0.09
Nodes (23): ChunkedCompositor, PlannedChunk, assemble, run, RenderNode, Run, .syntheticBase, Bool (+15 more)

### Community 245 - "ProjectLoadDamage"
Cohesion: 0.13
Nodes (18): LayerDamage, .isEmpty, .itemPhrase, .total, ProjectLoadDamage, .isDamaged, .itemCount, .summary (+10 more)

### Community 246 - "TransformChannelLogicTests"
Cohesion: 0.18
Nodes (5): CanvasManager, CGSize, UUID, TransformChannelLogicTests, .size

### Community 247 - ".draw"
Cohesion: 0.27
Nodes (11): CoreGraphicsCompositor, Bool, CGImage, CGRect, Double, Int, RenderNode, UIImage (+3 more)

### Community 248 - "FrameBakeStore"
Cohesion: 0.09
Nodes (23): Compression, Data, FrameBakeStore, .bakesRootDirectory, .cachesDirectory, .totalBytes, CGImage, Int (+15 more)

### Community 249 - "StructureSnapshot"
Cohesion: 0.17
Nodes (5): CanvasManager, StructureSnapshot, Int, Layer, guideStrokes

### Community 250 - "DerivedCelContent"
Cohesion: 0.32
Nodes (7): CelContentProvider, DerivedCelContent, AnyHashable, Cel, Int, RenderQuality, UIImage

### Community 251 - "StrokeStabilizer"
Cohesion: 0.32
Nodes (3): StrokeStabilizer, .stabilization, Double

### Community 252 - "String"
Cohesion: 0.32
Nodes (6): Entry, ObjectIdentifier, Set, String, UIGestureRecognizer, UIView

### Community 253 - "DabTarget"
Cohesion: 0.09
Nodes (20): AnyObject, BakedDab, BrushStamper, CollectingDabTarget, DabPose, .isIdentity, DabRNG, DiscardedDabTarget (+12 more)

### Community 254 - "ViewPreset"
Cohesion: 0.16
Nodes (9): CanvasManager, .activeViewName, Int, String, viewPresets, Bool, String, UUID (+1 more)

### Community 255 - "CanvasLayerFilterLogicTests"
Cohesion: 0.34
Nodes (5): CanvasLayerFilterLogicTests, Int, Set, String, URL

### Community 256 - "CanvasGeometryLogicTests"
Cohesion: 0.11
Nodes (6): CustomStringConvertible, CanvasGeometryLogicTests, Occurrence, .description, Int, String

### Community 257 - "StrokeGestureRecognizer"
Cohesion: 0.06
Nodes (29): StrokeGiveUp, handedOver, .inkSurvives, interrupted, StrokeInterruption, Bool, StrokeGestureRecognizer, Any (+21 more)

### Community 258 - ".testTheBarIsBlankOnceTheDocumentHasBaked"
Cohesion: 0.33
Nodes (6): Bool, String, TimeInterval, XCUIApplication, XCUIElement, TimelineBakeIndicationUITests

### Community 259 - "DrawingView"
Cohesion: 0.09
Nodes (20): CanvasNoticeBanner, .body, .icon, String, Void, DamagedSaveBanner, .body, Void (+12 more)

### Community 260 - "CanvasActiveLayer"
Cohesion: 0.20
Nodes (9): CanvasActiveLayer, .exists, .hasNoDrawingSurface, noDrawingSurface, none, raster, vector, Bool (+1 more)

### Community 261 - "Linear-light A/B — the picture for TODO item (10)"
Cohesion: 0.22
Nodes (8): 0. What these images are, and what they are not, 1. The formulas actually in the tree, 2. Did the symptom reproduce?, 3. Worst case, MEASURED, 4. What it does to existing artwork — scene 4 is the evidence, 5. Does the 256-entry-LUT parity argument survive?, 6. The images, Linear-light A/B — the picture for TODO item (10)

### Community 262 - "Coordinator"
Cohesion: 0.30
Nodes (6): Coordinator, CanvasManager, Int, Set, UUID, UITableViewDiffableDataSource

### Community 263 - "BakeWiringLogicTests"
Cohesion: 0.22
Nodes (8): BakeWiringLogicTests, .frameBytes, CanvasManager, Int, String, TimeInterval, URL, Void

### Community 264 - "Graph editor — screenshots"
Cohesion: 0.29
Nodes (6): Graph editor — screenshots, Reading them, The document that was authored, The shots, The three defects these shots found, before and after (2026-08-30), What is *not* in these shots

### Community 265 - "WindowEventTap"
Cohesion: 0.19
Nodes (9): AnyClass, NSObject, FoundElement, InstallReport, CGRect, UIEvent, WindowEventTap, UIAccessibilityTraits (+1 more)

### Community 266 - "CanvasPresentationModifier"
Cohesion: 0.27
Nodes (8): CanvasPresentationModifier, Bool, CanvasManager, Content, Void, View, PresentedContent, ViewModifier

### Community 267 - "PoseWidthAB"
Cohesion: 0.17
Nodes (13): CGAffineTransform, .affineOrLinearised, affine(), axisScales(), dabCount(), Pose, PoseWidthAB, similarityWidthScale() (+5 more)

### Community 268 - "LayerStackListView.Coordinator"
Cohesion: 0.24
Nodes (7): DropTarget, between, onto, LayerStackListView.Coordinator, TimeInterval, UIGestureRecognizerDelegate, UITableViewDelegate

### Community 269 - "Failure"
Cohesion: 0.18
Nodes (11): Error, Failure, bakeFailed, bakeStalled, couldNotWrite, noCanvas, nothingToExport, .sentence (+3 more)

### Community 270 - "BakeFixture"
Cohesion: 0.40
Nodes (5): BakeFixture, celArt, hold, noise, painted

### Community 271 - "Handoff"
Cohesion: 0.22
Nodes (9): A review found thirteen candidate defects and reached less than half the code, Do this first, Everything else open, Handoff, RENDER (29) is delivered through stage 6; stage 7 is what is left, Standing instructions from the owner about how you work, State, Traps this pass paid for (+1 more)

### Community 272 - "DistortLogicTests"
Cohesion: 0.17
Nodes (7): FloatingDistortDrag, DistortLogicTests, CATransform3D, CGSize, Int, UIImage, UInt8

### Community 273 - "FontResolveLogicTests"
Cohesion: 0.20
Nodes (5): FontResolveLogicTests, StubFontProvider, Bool, String, UIFont

### Community 274 - "Phase"
Cohesion: 0.18
Nodes (10): Phase, baking, failed, finished, .fraction, idle, .isRunning, writing (+2 more)

### Community 275 - "Coordinator"
Cohesion: 0.07
Nodes (26): LayerHostView, .maskedContentViews, Bool, CALayer, CGImage, Coordinator, .canvasContentScale, .sandwichPresentation (+18 more)

### Community 276 - "JSONValue"
Cohesion: 0.29
Nodes (7): JSONValue, bool, int, null, num, str, Bool

### Community 277 - "CodingKeys"
Cohesion: 0.09
Nodes (23): CodingKeys, animationGroupID, brush, color, composite, fill, fills, id (+15 more)

### Community 278 - ".sample"
Cohesion: 0.27
Nodes (9): ObjectiveC.runtime, ResolvedTarget, Bool, CGSize, Double, Int, UITouch, TouchSample (+1 more)

### Community 279 - "Atomic"
Cohesion: 0.60
Nodes (3): Atomic, .value, Value

### Community 280 - "CodingKeys"
Cohesion: 0.25
Nodes (8): CodingKeys, groups, guideIDs, localEdits, mode, references, spacing, t

### Community 281 - "TransformOverlaySupport.swift"
Cohesion: 0.28
Nodes (7): FloatingTransform, .effectiveScaleX, .effectiveScaleY, LayerTransform, .effectiveScaleX, .effectiveScaleY, OverlayTransformProjecting

### Community 282 - ".canvas"
Cohesion: 0.20
Nodes (7): DeferredVectorRender, Step, rasterize, showNow, Int, DeferredVectorRenderLogicTests, Bool

### Community 283 - "AnimationGroup"
Cohesion: 0.17
Nodes (12): AnimationGroup, CodingKeys, displayName, id, tagColor, Bool, CodableColor, Decoder (+4 more)

### Community 284 - "Alignment"
Cohesion: 0.29
Nodes (7): Alignment, center, .displayName, .id, justified, left, right

### Community 285 - "Resolution"
Cohesion: 0.29
Nodes (7): Resolution, .fraction, full, half, .id, quarter, .title

### Community 286 - "CGContextDabTarget"
Cohesion: 0.23
Nodes (8): CGGradient, CGContextDabTarget, DabGradientCache, Key, CGBlendMode, CGContext, Key, UIColor

### Community 287 - "ManifestSkeleton"
Cohesion: 0.47
Nodes (6): Decodable, Cel, Layer, ManifestSkeleton, Cel, Layer

### Community 288 - "Kind"
Cohesion: 0.40
Nodes (4): Kind, line, oval, rectangle

### Community 289 - "AppliedTool"
Cohesion: 0.40
Nodes (4): AppliedTool, Color, Double, Tool

### Community 290 - "Neighbourhood"
Cohesion: 0.40
Nodes (5): Neighbourhood, drawings, frames, .id, .title

### Community 291 - "Placement"
Cohesion: 0.40
Nodes (5): Placement, behind, .id, inFront, .title

### Community 292 - "Zone"
Cohesion: 0.40
Nodes (5): Zone, body, gap, leftHandle, rightHandle

### Community 293 - ".inkBounds"
Cohesion: 0.40
Nodes (3): CGRect, UIImage, .box

### Community 294 - "DistortUITests"
Cohesion: 0.32
Nodes (6): DistortUITests, Bool, CGRect, Double, TimeInterval, XCUIElement

### Community 295 - "CodingKey"
Cohesion: 0.22
Nodes (9): CodingKey, CodingKeys, color, String, CodingKeys, displayName, id, mode (+1 more)

### Community 296 - "Admission"
Cohesion: 0.50
Nodes (4): Admission, admitted, noHeadroom, overBudget

### Community 297 - "Kind"
Cohesion: 0.50
Nodes (4): Kind, compositorNode, group, layer

### Community 298 - "LiftKind"
Cohesion: 0.67
Nodes (3): LiftKind, lasso, wholeCel

### Community 299 - "2. The decisions"
Cohesion: 0.18
Nodes (11): 2. The decisions, Failure and partial completion: validate first, then a mutation that cannot fail, Memory and time: per-cel, bounded fan-out, and never `regenerateAllThumbnails`, One map, `M`, and every tier applies the same one, The coupling to TODO item (8) — fixed-point sample coordinates, The raster side is one widened parameter, The spacing floor: a large downscale changes how a stroke stamps, not only where, The vector/raster asymmetry is permanent, and undo has to say so (+3 more)

### Community 300 - "FillParams"
Cohesion: 0.18
Nodes (11): FillParams, edgeInset, edgeOverlap, fillColor, gapRadius, height, seedColor, seedX (+3 more)

### Community 303 - ".sampledColor"
Cohesion: 0.42
Nodes (3): CanvasManager, Bool, Color

### Community 304 - ".row"
Cohesion: 0.33
Nodes (6): MaskTuningSection, .body, ClosedRange, Float, String, Void

### Community 306 - "CodingKeys"
Cohesion: 0.29
Nodes (7): CodingKeys, activeCells, cellSize, cols, originX, originY, rows

### Community 307 - "RecordingDabTarget"
Cohesion: 0.33
Nodes (3): RecordingDabTarget, CGBlendMode, UIColor

### Community 308 - "Result"
Cohesion: 0.67
Nodes (3): Result, .isSuccess, Bool

### Community 309 - "RenderResolution"
Cohesion: 0.25
Nodes (7): RenderResolution, full, half, .id, .scale, threeQuarter, .title

### Community 311 - ".handleShouldReceive"
Cohesion: 0.36
Nodes (4): Bool, ObjectIdentifier, UIGestureRecognizer, UITouch

### Community 312 - "CanvasManager"
Cohesion: 0.06
Nodes (38): CanvasManager, .artworkSize, .preciseStrokeCount, .resizableArtworkExtentRange, .spacingFloorSurvey, CanvasResizeAudit, .canProceed, .celCount (+30 more)

### Community 313 - ".testTheCanvasComesBackToTheBakedFrameAfterAStroke"
Cohesion: 0.31
Nodes (7): BakeWiringUITests, CGVector, String, TimeInterval, UInt8, XCUIApplication, XCUIElement

### Community 314 - "LayerRowModel"
Cohesion: 0.15
Nodes (12): String, UIColor, LayerRowModel, .folderID, .isFolder, .maskSource, BlendMode, CGRect (+4 more)

### Community 315 - "PackedSampleRun"
Cohesion: 0.05
Nodes (28): Float32, CodingKeys, d, endPoint, kind, o, p, rotation (+20 more)

### Community 373 - "RestSpaceDabBakeLogicTests"
Cohesion: 0.16
Nodes (10): rest, VectorStroke, BrushStamper, RestSpaceDabBakeLogicTests, BrushStamper, CGAffineTransform, Int, String (+2 more)

## Ambiguous Edges - Review These
- `Stroke-delivery regression (pencil-only-drawing default)` → `Fill tool off-center fill vertically mirrored (regression)`  [AMBIGUOUS]
  BUGS.md · relation: conceptually_related_to

## Knowledge Gaps
- **1531 isolated node(s):** `Do this first`, `State`, `RENDER (29) is delivered through stage 6; stage 7 is what is left`, `A review found thirteen candidate defects and reached less than half the code`, `What this pass established, and would otherwise be re-derived` (+1526 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **22 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **What is the exact relationship between `Stroke-delivery regression (pencil-only-drawing default)` and `Fill tool off-center fill vertically mirrored (regression)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `CGFloat` connect `CGFloat` to `PaintUITestCase`, `ShapeGeometry`, `Coordinator`, `PointCloudIndex`, `CanvasManager`, `Homography`, `ShapeOverlayView`, `cels`, `StrokeCanvasView`, `ActionRecorder`, `TimelineGraphBand`, `CGPoint`, `VectorEraserLogicTests`, `LassoMoveLogicTests`, `.rasterize`, `CanvasManager`, `TextHitTestLogicTests`, `PerfBaselineTests`, `Lattice`, `StrokeScratch`, `Binding`, `VectorFloat`, `FloatingPieceOverlayView`, `AnimationTimeline`, `ProjectSaveLogicTests`, `LayerStackCell`, `VectorCanvas`, `VectorCanvasData`, `StrokeGeometryLogicTests`, `TextRecipe`, `TimelineKeyMarkersLogicTests`, `ObjectTransformFrame`, `TextOverlayView`, `XCUIApplication`, `BrushEngineLogicTests`, `AlphaMask`, `ProjectStore`, `FrameRecipe`, `SideToolbar`, `CompositorParityLogicTests`, `CanvasManager`, `EffectSettingsBar`, `ColorMathOklabLogicTests`, `ObjectTransformOverlayView`, `CompositorMetalEngine`, `.makeUIView`, `SandwichLogicTests`, `VectorCanvasDataLogicTests`, `TextBakeCharacterizationTests`, `InterpolationMotionGroupLogicTests`, `.solidImage`, `DeformFactorization`, `.evaluate`, `InterpolationRenderLogicTests`, `ObjectTransformDrag`, `GuideOverlayView`, `BrushBlendMode`, `LassoFillLogicTests`, `GuidePath`, `.group`, `VectorSample`, `InterpolationGuideLogicTests`, `TextTransformOverlayView`, `FontDescriptor`, `CanvasManager`, `linear_light_q1q2.swift`, `Typography`, `Codable`, `InterpolateBar`, `XCUIApplication`, `VectorEraserHybridLogicTests`, `BakeKeyEncoder`, `CanvasResizeLogicTests`, `.assertLinear`, `OnionSkinSettingsSource`, `LayerStackListView`, `CanvasManager`, `PinchMergeGateLogicTests`, `.rasterize`, `RasterLayerTexture`, `CanvasTransformFreezeUITests`, `.lassoFill`, `CurveEditor`, `StrokeSpatialIndex`, `Int`, `TextLayout`, `.compositeSize`, `TransformChannelID`, `WarpAgreementCharacterizationTests`, `.rgbaBytes`, `ObjectTransformLogicTests`, `CanvasResizeSheet`, `TextSettingsPanel`, `String`, `PlacedImageShapeLogicTests`, `UInt8`, `TimelineLayoutKeyLogicTests`, `InterpolationEngineDiagnosticsLogicTests`, `VideoFrameWriter`, `TimelineRowLayoutLogicTests`, `CelContentProviderLogicTests`, `SizePreviewWindow`, `linear_light_ab.swift`, `SpacingChart`, `TransformTrack`, `Brush`, `TimelineLayoutKey`, `FrameBaker`, `.previewed`, `TimelineBakeBar`, `ARAPLogicTests`, `PoseQuad`, `.assertChunkedMatchesWhole`, `TransformChannelLogicTests`, `.draw`, `StrokeStabilizer`, `DabTarget`, `DrawingView`, `Coordinator`, `WindowEventTap`, `PoseWidthAB`, `DistortLogicTests`, `FontResolveLogicTests`, `Coordinator`, `JSONValue`, `CodingKeys`, `.sample`, `TransformOverlaySupport.swift`, `Resolution`, `CGContextDabTarget`, `Kind`, `AppliedTool`, `RecordingDabTarget`, `RenderResolution`, `LayerTransform`, `CanvasManager`, `LayerRowModel`, `PackedSampleRun`, `.testGrainTravelsWithTheInkUnderAPose`, `RestSpaceDabBakeLogicTests`?**
  _High betweenness centrality (0.292) - this node is a cross-community bridge._
- **Why does `CGPoint` connect `CGPoint` to `ShapeGeometry`, `Coordinator`, `PointCloudIndex`, `CanvasManager`, `Homography`, `ShapeOverlayView`, `cels`, `StrokeCanvasView`, `TimelineGraphBand`, `VectorEraserLogicTests`, `LassoMoveLogicTests`, `.rasterize`, `CanvasManager`, `TextHitTestLogicTests`, `PerfBaselineTests`, `Lattice`, `StrokeScratch`, `VectorFloat`, `FloatingPieceOverlayView`, `AnimationTimeline`, `ProjectSaveLogicTests`, `VectorCanvas`, `ColorPickerPanel`, `SelectionOverlayView`, `VectorCanvasData`, `StrokeGeometryLogicTests`, `TextRecipe`, `ObjectTransformFrame`, `TextOverlayView`, `BrushEngineLogicTests`, `AlphaMask`, `ProjectStore`, `CanvasManager`, `EffectSettingsBar`, `SaveDamageGateLogicTests`, `ColorMathOklabLogicTests`, `ObjectTransformOverlayView`, `.makeUIView`, `SandwichLogicTests`, `VectorCanvasDataLogicTests`, `TextBakeCharacterizationTests`, `InterpolationMotionGroupLogicTests`, `.solidImage`, `DeformFactorization`, `.evaluate`, `InterpolationRenderLogicTests`, `ObjectTransformDrag`, `GuideOverlayView`, `BrushBlendMode`, `LassoFillLogicTests`, `GuidePath`, `.group`, `VectorSample`, `InterpolationGuideLogicTests`, `TextTransformOverlayView`, `linear_light_q1q2.swift`, `Typography`, `EyedropperLogicTests`, `Codable`, `VectorEraserHybridLogicTests`, `CanvasResizeLogicTests`, `.assertLinear`, `FillGestureRestartLogicTests`, `CanvasManager`, `.rasterize`, `RasterLayerTexture`, `CanvasManager`, `CurveEditor`, `StrokeSpatialIndex`, `TextLayout`, `.setUpGestures`, `TransformChannelID`, `WarpAgreementCharacterizationTests`, `.rgbaBytes`, `ObjectTransformLogicTests`, `String`, `PlacedImageShapeLogicTests`, `TimelineLayoutKeyLogicTests`, `InterpolationEngineDiagnosticsLogicTests`, `CelContentProviderLogicTests`, `XCTestCase`, `linear_light_ab.swift`, `Brush`, `CGFloat`, `ARAPLogicTests`, `PoseQuad`, `TransformChannelLogicTests`, `StrokeStabilizer`, `DabTarget`, `WindowEventTap`, `PoseWidthAB`, `LayerStackListView.Coordinator`, `DistortLogicTests`, `Coordinator`, `.sample`, `TransformOverlaySupport.swift`, `CGContextDabTarget`, `Kind`, `DistortUITests`, `.draw`, `.sampledColor`, `RecordingDabTarget`, `LayerTransform`, `CanvasManager`, `PackedSampleRun`, `.testGrainTravelsWithTheInkUnderAPose`, `RestSpaceDabBakeLogicTests`?**
  _High betweenness centrality (0.204) - this node is a cross-community bridge._
- **Why does `XCTestCase` connect `XCTestCase` to `ShapeGeometry`, `ProjectBackupManager`, `.manager`, `Homography`, `ValueLayerLogicTests`, `cels`, `TimelineGraphBand`, `CGPoint`, `UIKit`, `VectorEraserLogicTests`, `LassoMoveLogicTests`, `Effect`, `.drawLine`, `TextHitTestLogicTests`, `PerfBaselineTests`, `Lattice`, `ProjectSaveLogicTests`, `SelectionOverlayView`, `VectorCanvasData`, `StrokeGeometryLogicTests`, `TextRecipe`, `TimelineKeyMarkersLogicTests`, `FrameExportLogicTests`, `BrushEngineLogicTests`, `AlphaMask`, `FrameRecipe`, `CompositorParityLogicTests`, `.assertChunkedMatchesWholeOnGPU`, `GalleryOpenState`, `SaveDamageGateLogicTests`, `LayerTreeCharacterizationTests`, `ColorMathOklabLogicTests`, `KeyframeControlLogicTests`, `SandwichLogicTests`, `VectorCanvasDataLogicTests`, `TextBakeCharacterizationTests`, `InterpolationMotionGroupLogicTests`, `.solidImage`, `EffectMultiPassLogicTests`, `InterpolationRenderLogicTests`, `PlaybackBoundsCharacterizationTests`, `LassoFillLogicTests`, `EffectParityLogicTests`, `VectorSample`, `InterpolationGuideLogicTests`, `Typography`, `EyedropperLogicTests`, `Codable`, `VectorEraserHybridLogicTests`, `CanvasResizeLogicTests`, `CanvasNotice`, `FillGestureRestartLogicTests`, `PinchMergeGateLogicTests`, `ShapeHoldClock`, `BakeQueueLogicTests`, `VectorPreviewPlanLogicTests`, `EffectParameterTrackLogicTests`, `WarpAgreementCharacterizationTests`, `AnimationCurveLogicTests`, `CanvasTouchOwnerLogicTests`, `.rgbaBytes`, `ObjectTransformLogicTests`, `FillBoundaryLogicTests`, `PlacedImageShapeLogicTests`, `TimelineLayoutKeyLogicTests`, `InterpolationEngineDiagnosticsLogicTests`, `TimelineRowLayoutLogicTests`, `CelContentProviderLogicTests`, `BlockDragCharacterizationTests`, `TimelineGraphChannelListLogicTests`, `TransformTrack`, `DecodedFrameRing`, `FrameBakerLogicTests`, `FrameBakeStoreLogicTests`, `.previewed`, `OnionSkinLogicTests`, `TimelineBakeBar`, `ARAPLogicTests`, `PoseQuad`, `MemoryBudgetLogicTests`, `.assertChunkedMatchesWhole`, `TransformChannelLogicTests`, `CanvasLayerFilterLogicTests`, `CanvasGeometryLogicTests`, `StrokeGestureRecognizer`, `BakeWiringLogicTests`, `DistortLogicTests`, `FontResolveLogicTests`, `.canvas`, `PackedSampleRun`, `RestSpaceDabBakeLogicTests`?**
  _High betweenness centrality (0.108) - this node is a cross-community bridge._
- **Are the 123 inferred relationships involving `CGPoint` (e.g. with `.init()` and `.finish()`) actually correct?**
  _`CGPoint` has 123 INFERRED edges - model-reasoned connections that need verification._
- **Are the 24 inferred relationships involving `CGFloat` (e.g. with `.draw()` and `.append()`) actually correct?**
  _`CGFloat` has 24 INFERRED edges - model-reasoned connections that need verification._
- **Are the 4 inferred relationships involving `Effect` (e.g. with `.channelIDs()` and `.all`) actually correct?**
  _`Effect` has 4 INFERRED edges - model-reasoned connections that need verification._