# Graph Report - vector-interpolation-keyframes-d484df  (2026-08-07)

## Corpus Check
- 145 files · ~330,951 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3594 nodes · 10583 edges · 141 communities (131 shown, 10 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 1304 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7045da32`
...
7. `layers` - 86 edges
8. `CanvasManager` - 76 edges
9. `Coordinator` - 75 edges
...
Cohesion: 0.11
Nodes (28): Identifiable, CodableColor, .uiColor, ElementData, fill, image, stroke, ImageRef (+20 more)
...
Cohesion: 0.08
Nodes (29): CGPathElementType, Direction, backward, forward, fromRest, Evaluation, GroupWarp, InterpolationEvaluator (+21 more)
...
Cohesion: 0.10
Nodes (16): CanvasView, Coordinator, CanvasManager, CGSize, Coordinator, Date, NSLayoutConstraint, TimeInterval (+8 more)
...
Cohesion: 0.13
Nodes (9): Capsule, .boundingBox, StrokeGeometry, Bool, CGRect, ClosedRange, Int, samples (+1 more)

### Community 23 - "String"
Cohesion: 0.06
Nodes (40): Layer, Bool, Cel, Double, String, UIImage, UUID, CanvasManager (+32 more)
...
### Community 30 - "layers"
Cohesion: 0.17
Nodes (8): .activeLayerIsVector, .activeCelIsInBetween, Bool, CanvasManager, Bool, Int, Void, layers
...
Cohesion: 0.06
Nodes (31): 0. The brief, 10. Decisions and remaining questions, 11. How clear is the vision, 12. Sources, 1. The central problem, 2. What the codebase already gives us, 4. The load-bearing decision: an inbetween is *derived*, never *stored*, 6.1 What they are (+23 more)
...
Cohesion: 0.17
Nodes (5): LayerHostView, NSCoder, Context, LayerTransform, UIImageView
...
Cohesion: 0.05
Nodes (38): CodingKey, CodingKeys, boundGroups, id, interval, role, samples, CodingKeys (+30 more)
...
### Community 79 - "View"
Cohesion: 0.29
Nodes (8): .body, MotionGroupRow, .body, .colourBakeButton, .wholeFrameNote, CanvasManager, String, View

### Community 80 - "5. Carry-overs"
Cohesion: 0.08
Nodes (25): 5.10 For Phase 5's motion groups, 5.11 What the papers do — and where they do not help, 5.12 Where a liquify at *t* is stored — §8 item 25's design question, answered, 5.13 "Lasso transform at *t*" — what it turned out to be, and why it is a refusal, 5.14 Commit's fidelity — the decision, and what it costs, 5.15 Run `uptime` before diagnosing a failed full run — it is the cheapest signal and no earlier entry mentions it, 5.7 The lattice encoding — *answered, kept for the constraint*, 5.8 For Phase 3's evaluator (+17 more)
...
Cohesion: 0.07
Nodes (24): CanvasManager, .hasAnonymousWholeFrameGroup, .interpolationCommitOptions, .interpolationContentProvider, .interpolationKeyframes, .interpolationOptions, .interpolationReferenceCanvases, .interpolationTarget (+16 more)
...
### Community 92 - "Brush"
Cohesion: 0.17
Nodes (5): Brush, Sweep, Bool, ClosedRange, Double
...
Cohesion: 0.17
Nodes (13): InterpolationRefusal, alreadyInterpolated, interpolationNotEvaluable, .message, notAVectorLayer, notEnoughReferences, nothingToCommit, nothingToReproject (+5 more)
...
### Community 110 - ".draw"
Cohesion: 0.18
Nodes (9): ContentProvider, CGSize, UIImage, image, RenderQuality, full, preview, CGContext (+1 more)
...
Cohesion: 0.12
Nodes (17): InterpolationRecipe, .isWellFormed, .referencedCels, InterpolationReference, Kind, easeIn, easeInOut, easeOut (+9 more)
...
### Community 133 - "CGFloat"
Cohesion: 0.13
Nodes (9): bendRatio(), cellSize(), coverage(), cShape(), polyline(), Int, Void, CGFloat (+1 more)
...
## Knowledge Gaps
- **579 isolated node(s):** `graphify-guard.sh script`, `gallery`, `sizePicker`, `editor`, `softRound` (+574 more)
...
- **Why does `CGFloat` connect `CGFloat` to `.launchIntoEditor`, `VectorEraserHybridLogicTests`, `.manager`, `.setCanvasPadding`, `TimelineRowView`, `BrushGrain`, `CanvasManager`, `StrokeGeometryLogicTests`, `Deterministic`, `VectorEraserLogicTests`, `VectorCanvas`, `BrushEngineLogicTests`, `VectorStroke`, `CanvasManager`, `StrokeCanvasView`, `InterpolationRenderLogicTests`, `PointCloudIndex`, `Coordinator`, `.transparentFormat`, `CodingKeys`, `StrokeGeometry`, `String`, `CanvasManager`, `cels`, `CGPoint`, `PerfBaselineTests`, `StrokeSpatialIndex`, `StrokeSettingsPanel`, `AnimationTimeline`, `.load`, `Lattice`, `InterpolationModelLogicTests`, `BrushDynamics`, `RasterLayerTexture`, `ShapeGeometry`, `DeformFactorization`, `.makeUIView`, `Color`, `CGContextDabTarget`, `LayerStackCell`, `SideToolbar`, `.restLattice`, `VectorEraserMode`, `ShapeDetectorLogicTests`, `LayerRowModel`, `StrokeStabilizer`, `.stampStroke`, `LayerStackListView.Coordinator`, `EraserSettingsPanel`, `CanvasManager`, `Equatable`, `InterpolationEngineDiagnosticsLogicTests`, `DabLattice`, `UIKit`, `DrawingView`, `Brush`, `InterpolateBar`, `ShapeOverlayView`, `ShapeDetector`, `.draw`, `BrushBlendMode`, `UIView`, `ActionsMenu`, `InterpolationRecipe`, `InterpolationPreviewKey`, `ARAPLogicTests`?**
  _High betweenness centrality (0.300) - this node is a cross-community bridge._
- **Why does `CGPoint` connect `CGPoint` to `Edge`, `VectorEraserHybridLogicTests`, `.setCanvasPadding`, `CGFloat`, `BrushGrain`, `CanvasManager`, `StrokeGeometryLogicTests`, `ColorPickerPanel`, `VectorEraserLogicTests`, `VectorCanvas`, `BrushEngineLogicTests`, `HandleKind`, `CanvasManager`, `StrokeCanvasView`, `InterpolationRenderLogicTests`, `PointCloudIndex`, `Coordinator`, `.transparentFormat`, `TimelineRowView`, `StrokeGeometry`, `String`, `CanvasManager`, `cels`, `VectorElement`, `PerfBaselineTests`, `layers`, `StrokeSpatialIndex`, `AnimationTimeline`, `FloatingPieceOverlayView`, `Lattice`, `InterpolationModelLogicTests`, `ProjectSaveLogicTests`, `SelectionOverlayView`, `RasterLayerTexture`, `Foundation`, `ShapeGeometry`, `DeformFactorization`, `.makeUIView`, `CGContextDabTarget`, `.restLattice`, `ObjectTransformOverlayView`, `ShapeDetectorLogicTests`, `StrokeStabilizer`, `.stampStroke`, `LayerStackListView.Coordinator`, `CanvasManager`, `Equatable`, `InterpolationEngineDiagnosticsLogicTests`, `UIKit`, `Brush`, `ShapeOverlayView`, `ShapeDetector`, `.draw`, `BrushBlendMode`, `UIView`, `ARAPLogicTests`?**
  _High betweenness centrality (0.205) - this node is a cross-community bridge._
- **Why does `CanvasManager` connect `CanvasManager` to `CGFloat`, `String`, `CanvasManager`, `MetalFillEngine`, `layers`, `CanvasManager`, `ViewPreset`, `RasterLayerTexture`, `SwiftUI`, `ShapeGeometry`, `PerfMonitor`, `Codable`, `VectorEraserMode`, `UndoHistory`, `Equatable`, `CopiedCel`, `Brush`, `StructureSnapshot`, `MotionGroup`?**
